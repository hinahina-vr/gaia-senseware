...
const proceduralAudioSource = read("src/exploration/procedural-audio.js");
...
assert.equal(content.modes.length, 8, "Earth catalog must contain exactly 8 exhibits");
assert.equal(content.INTRO_MODE_CHOICES.length, 8, "Entrance must contain exactly 8 exhibits");
assert.deepEqual(Array.from(content.modes, ({ id }) => id), expectedIds);
assert.equal(new Set(content.modes.map(({ id }) => id)).size, 8);
...
assert.match(content.modes[4].description, /夜間光画素.*地図上の位置.*赤い円/u);
...
assert.match(appSource, /const MODE_COUNT = 8;/u);
...
assert.match(liveExhibitsSource, /gaia:live-touch[\s\S]*button class="gaia-live-exhibit-touch-hint"[\s\S]*data-live-light-touch[\s\S]*光に触れる[\s\S]*TOUCH \/ DRAG/u);
assert.match(liveExhibitsSource, /data-live-light-touch[\s\S]*addLightTouch\(x, y, 1\.35\)/u);
assert.match(liveExhibitsSource, /data-live-stage="observe"[\s\S]*data-live-stage="locate"[\s\S]*data-live-stage="visualize"[\s\S]*data-live-stage="sonify"/u);
assert((liveExhibitsSource.match(/<svg viewBox="0 0 64 64">/gu) || []).length >= 4, "live transformation stages require graphic symbols");
assert.match(liveExhibitsSource, /data-live-exhibit-input[\s\S]*data-live-exhibit-location[\s\S]*data-live-exhibit-visual-map[\s\S]*data-live-exhibit-sound-map/u);
assert.match(liveExhibitsSource, /data-live-sound-toggle[\s\S]*data-live-sound-label>展示音を再生/u);
assert.match(liveExhibitsSource, /activeIndex = index;[\s\S]*classList\.add\("is-live-exhibit"\)[\s\S]*proceduralAudio\.setFocus/u);
assert.match(proceduralAudioSource, /MODE_PROFILES[\s\S]*wind-field[\s\S]*carbon-pulse[\s\S]*rain-chorus[\s\S]*no2-veil/u);
assert.match(proceduralAudioSource, /createConvolver\(\)[\s\S]*scheduleSequence[\s\S]*playSequenceNote[\s\S]*triggerTouch/u);
assert.match(proceduralAudioSource, /gaia:live-touch[\s\S]*detail\.x[\s\S]*detail\.y[\s\S]*x \* 1\.6 - 0\.8/u);
...
assert.match(html, /gaia-mode-loader\.js\?v=gaia-sound-return-layout-1/u);
assert.match(modeLoaderSource, /map-ui-grid-polish\.css\?v=gaia-mobile-exhibit-hud-5/u);
assert.match(modeLoaderSource, /map-ui-grid-polish\.js\?v=gaia-mobile-exhibit-hud-5/u);
assert.match(modeLoaderSource, /app-content\.js\?v=gaia-abstract-entry-1/u);
assert.match(modeLoaderSource, /app\.js\?v=gaia-no-bare-exhibit-1/u);
assert.match(modeLoaderSource, /particles-v9\.js\?v=gaia-light-surface-fps-1/u);
assert.match(appSource, /const mapExhibitIsVisible = japanIsOpen[\s\S]*mapSurface === "map"[\s\S]*!japanLayer\.classList\.contains\("is-live-exhibit"\)/u);
...
assert.match(modeLoaderSource, /src\/exploration\/index\.js\?v=gaia-mobile-exhibit-hud-5/u);
...
assert.match(modeLoaderSource, /styles\.css\?v=gaia-gx-feature-center-1/u);
assert.match(html, /id="map-mode-01-tooltip"[\s\S]*どの信号を、地図で読みますか？[\s\S]*CO₂が季節ごとに上下しながら/u);
assert.doesNotMatch(html, /data-intro-path="abstract"/u);
assert.match(html, /id="map-surface-map"[\s\S]*id="map-surface-light"[\s\S]*id="abstract-mode-list"/u);
assert.match(html, /id="map-mode-light-tooltip"[\s\S]*どの感覚に、触れますか？[\s\S]*地球が発している8つの信号/u);
assert.match(appSource, /const setMapSurface = \(surface[\s\S]*classList\.toggle\("is-abstract-exhibit"[\s\S]*abstractModeList\.hidden/u);
assert.match(appSource, /abstractModeButton\.className = "map-mode-button map-mode-button--light"[\s\S]*abstractModeList\.append/u);
assert.match(stylesSource, /\.experience\.japan-open:has\(\.japan-layer\.is-abstract-exhibit\) > #gaia-canvas[\s\S]*pointer-events: auto !important/u);
assert.match(appSource, /if \(path === "map"\)[\s\S]{0,440}openJapan\(\{ respectUrlMode: false, focusModeBank: true \}\)/u);
assert.match(appSource, /aria-describedby", "map-mode-01-tooltip"/u);
...
assert.match(html, /このデータの出典を表示する/u);
