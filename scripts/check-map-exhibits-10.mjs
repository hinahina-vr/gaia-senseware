...
assert.doesNotMatch(appSource, /ctx\.fillText\(`M\$\{event\.magnitude\.toFixed\(1\)\}`/u);
assert.match(appSource, /"callout-only"/u);
...
assert.match(html, /gaia-mode-loader\.js\?v=gaia-current-land-foreground-1/u);
assert.match(modeLoaderSource, /map-ui-grid-polish\.css\?v=gaia-dock-step-width-1/u);
assert.match(modeLoaderSource, /map-ui-grid-polish\.js\?v=gaia-estat-picker-1/u);
assert.match(modeLoaderSource, /app-content\.js\?v=gaia-map-preview-copy-1/u);
assert.match(modeLoaderSource, /app\.js\?v=gaia-current-land-foreground-1/u);
assert.match(modeLoaderSource, /map-instrument-ui\.css\?v=gaia-poi-compact-card-1/u);
assert.match(modeLoaderSource, /styles\.css\?v=gaia-guide-copy-three-lines-1/u);
...
assert.match(stylesSource, /\.intro-entry-guide-shade \{[\s\S]{0,760}0 0 0 9999px rgba\(0, 5, 18, 0\.88\)/u);
assert.match(stylesSource, /\.intro-entry-guide-bubble \{[\s\S]*filter: blur\(8px\);[\s\S]*translateY\(18px\) scale\(0\.965\)/u);
assert.match(stylesSource, /\.intro-entry-guide-bubble \{[\s\S]*transition: opacity 680ms 100ms ease, filter 780ms 100ms ease, transform 820ms 100ms/u);
...
assert.match(modeLoaderSource, /src\/exploration\/index\.js\?v=gaia-warm-stat-map-1/u);
assert.match(modeLoaderSource, /firms-exhibit\.css\?v=gaia-firms-no-return-1/u);
assert.match(modeLoaderSource, /planet-signals-exhibit\.css\?v=gaia-satellite-clouds-1/u);
assert.match(modeLoaderSource, /map-chapter-navigation\.css\?v=gaia-fixed-chapter-arrows-1/u);
...
assert.match(appSource, /const expansive = rect\.width >= 2400;/u);
assert.match(appSource, /compact \? 16 : \(expansive \? 30 : 18\)/u);
...
assert.match(appSource, /dataset\.selectionLabelTailSide = tailOnTop \? "top" : tailOnLeft \? "left" : "right"/u);
assert.match(appSource, /dataset\.selectionLabelTailLengthPx/u);
assert.match(appSource, /\{ prominent: true, compactProminent: true, anchor: point, allowDuringPlotReveal: true,[\s\S]{0,180}preferBelow: true/u);
...
assert.match(html, /id="map-mode-preview"[\s\S]*id="map-mode-preview-number">01 \/ AIR<[\s\S]*id="map-mode-preview-label">地球の一呼吸<[\s\S]*1958年からの濃度の変化をたどります。/u);
