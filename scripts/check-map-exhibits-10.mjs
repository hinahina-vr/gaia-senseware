...
assert.match(liveExhibitsSource, /gaia:live-touch[\s\S]*gaia-live-exhibit-touch-hint[\s\S]*光に触れる[\s\S]*TOUCH \/ DRAG/u);
...
assert.match(stylesSource, /\.gaia-live-exhibit-primary > strong[\s\S]*clamp\(48px, 2\.35vw, 90px\)/u);
...
assert.match(proceduralAudioSource, /Math\.min\(0\.38, 0\.1 \+ Math\.sqrt\(volume\) \* 0\.28\)/u);
assert.match(mapGridStylesSource, /\.japan-layer\.is-live-exhibit \.map-grid-bank[\s\S]*width: clamp\(400px, 22vw, 720px\)/u);
...
assert.match(html, /gaia-mode-loader\.js\?v=gaia-no-breathing-flash-1/u);
assert.match(modeLoaderSource, /map-ui-grid-polish\.css\?v=gaia-title-only-1/u);
...
assert.match(modeLoaderSource, /app\.js\?v=gaia-no-breathing-flash-1/u);
assert.match(modeLoaderSource, /src\/exploration\/index\.js\?v=gaia-live-explained-audio-1/u);
...
assert.match(modeLoaderSource, /styles\.css\?v=gaia-live-explained-audio-1/u);
...
assert.match(stylesSource, /\.japan-layer\.is-live-exhibit \.japan-overlay \{[\s\S]*opacity: 0\.22;[\s\S]*visibility: visible;/u);
