...
assert.match(html, /gaia-mode-loader\.js\?v=gaia-story-return-cycle-1/u);
...
assert.match(modeLoaderSource, /app\.js\?v=gaia-map-panel-collision-1/u);
...
assert.match(modeLoaderSource, /styles\.css\?v=gaia-six-fixes-1/u);
...
assert.match(html, /class="map-mode-groups"[\s\S]*id="japan-mode-list"[\s\S]*id="map-light-overlay-open"/u);
assert.match(html, /id="map-light-overlay"[\s\S]*id="abstract-mode-list"/u);
...
assert.match(stylesSource, /\.japan-layer\.is-abstract-exhibit > #gaia-canvas[\s\S]*pointer-events: none !important[\s\S]*mix-blend-mode: screen/u);
