...
assert.deepEqual(categories.map(category => category.label), ["気候と炭素", "空と天気", "水と森", "人口と暮らし", "資源とエネルギー", "大地の活動"]);
...
assert.equal(categorySandbox.GaiaMapCategories.get("12").id, "water");
...
assert.match(html, /gaia-mode-loader\.js\?v=gaia-weather-credit-2/u);
assert.match(modeLoaderSource, /map-ui-grid-polish\.css\?v=gaia-exhibit-preview-card-1/u);
assert.match(modeLoaderSource, /map-ui-grid-polish\.js\?v=gaia-subject-categories-1/u);
...
assert.match(modeLoaderSource, /app-content\.js\?v=gaia-ecologies-reading-1/u);
assert.match(modeLoaderSource, /app\.js\?v=gaia-ecologies-reading-1/u);
...
assert.match(modeLoaderSource, /src\/exploration\/index\.js\?v=gaia-weather-credit-2/u);
assert.match(modeLoaderSource, /map-exhibit-actions\.css\?v=gaia-unified-dock-height-1/u);
assert.match(modeLoaderSource, /statistics-lab\.js\?v=gaia-statistics-byok-1/u);
...
assert.match(modeLoaderSource, /map-chapter-navigation\.css\?v=gaia-weather-credit-2/u);
...
assert.match(appSource, /selectionLabelShape: "inline-readout"/u);
assert.match(appSource, /drawRainSelectionLabel\(point, row, getForestRainRadius\(row\.precipitationMmDay\)\)/u);
assert.match(appSource, /ctx\.fillText\(text, textX, baseline\)/u);
