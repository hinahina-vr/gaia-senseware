...
assert.match(html, /INSTALLATION BANK \/ MAP 01—12 \+ LIGHT 01—08/u);
...
assert.match(html, /gaia-mode-loader\.js\?v=gaia-character-main-card-1/u);
assert.match(modeLoaderSource, /map-ui-grid-polish\.css\?v=gaia-unified-world-bank-1/u);
...
assert.match(modeLoaderSource, /app\.js\?v=gaia-story-return-state-2/u);
...
assert.match(modeLoaderSource, /styles\.css\?v=gaia-character-main-card-1/u);
assert.match(html, /id="map-mode-preview"[\s\S]*どの信号を、地図で読みますか？[\s\S]*CO₂が季節ごとに上下しながら/u);
...
assert.match(html, /class="map-mode-groups"[\s\S]*id="japan-mode-list"[\s\S]*id="abstract-mode-list"/u);
assert.match(appSource, /surface === "light" \? "どの感覚に、触れますか？"[\s\S]*INTRO_PATHS\.abstract\.lead/u);
assert.match(appSource, /const setMapSurface = \(surface[\s\S]*classList\.toggle\("is-abstract-exhibit"/u);
