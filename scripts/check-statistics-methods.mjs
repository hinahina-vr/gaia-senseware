...
assert.deepEqual(METHOD_GROUPS.map(group => group.name), ["記述・探索", "確率分布", "標本・推定", "仮説検定", "回帰分析", "ベイズ推論", "総合演習"]);
assert.equal(METHOD_LOOKUP.size, 25);
assert.equal(METHOD_GROUPS.flatMap(group => group.methods).length, 25, "A method was duplicated or removed");
...
console.log("PASS statistics categories: seven purpose-based groups, all 25 methods retained, legacy action targets preserved, no visible lecture numbers");
