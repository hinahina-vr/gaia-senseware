...
assert.match(index, /id="gaia-opening-entry-continue"/u);
assert.match(index, /id="gaia-opening-entry-story"/u);
assert.match(index, /id="gaia-opening-entry-explore"/u);
assert.match(index, /id="gaia-opening-entry-sound-toggle"/u);
...
assert.match(index, /○ SOURCE \/ 公開記録/u);
assert.match(index, /△ DERIVED \/ 計算・補間/u);
assert.match(index, /◇ SCENARIO \/ 仮定・操作/u);
assert.match(opening, /gaiaSenseware:entryPreference:v1/u);
assert.match(opening, /lastRoute: "tour"/u);
...
assert.match(novel, /source === "entry-continue"/u);
assert.match(novel, /openManualArchive\("load"\)/u);
...
console.log(JSON.stringify({ status: "passed", guide: "docs/CONTEST_2026_SUBMISSION.md", architecture: "docs/ARCHITECTURE.md", workflow: ".github/workflows/contest-checks.yml", checks: 56 }, null, 2));
