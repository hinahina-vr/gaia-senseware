// Generated from story/物語台本.md by scripts/build-novel-story.mjs. Do not edit by hand.
globalThis.GAIA_NOVEL_STORY_V6 = Object.freeze({
  "storyVersion": 6,
  "title": "GAIA SENSATION",
  "systemTitle": "GAIA SENSEWARE",
  "startSceneId": "current_notice",
  "saveFields": [
    "storyVersion",
    "stepId",
    "reachedSceneIds",
    "viewed",
    "evesRoute",
    "observationOrder",
    "editorialChoice",
    "visitorAction",
    "audio",
    "readStepIds",
    "clear",
    "archivesUnlocked",
    "sessionId"
  ],
  "visitorInput": {
    "maxLength": 120,
    "persistent": false
  },
  "requiredSceneIds": [
    "current_notice",
    "opening_empty_seat",
    "prologue_online_circle",
    "prologue_basil",
    "choice_observation_order",
    "first_meeting_promise",
    "first_meeting_hall",
    "festival_walk",
    "production_year",
    "absence",
    "search",
    "festival_build",
    "gx_deep_time",
    "mode03_map",
    "mode07_abstract",
    "interlude_sea",
    "mode08_map_layers",
    "mode10_space",
    "choice_editorial",
    "epilogue_visitor_field",
    "choice_visitor_action",
    "final_record",
    "return_to_start"
  ],
  "requiredInteractions": [
    "gx",
    "map03",
    "abstract07",
    "map08",
    "space10"
  ],
  "finalResults": [
    "SOURCE_RECORD×WRITE",
    "SOURCE_RECORD×LEAVE_EMPTY",
    "DISCLOSE_DERIVATION×WRITE",
    "DISCLOSE_DERIVATION×LEAVE_EMPTY"
  ],
  "generationDetails": {
    "referencePostCount": "制作ログに件数記録なし",
    "similarPostCount": "制作ログに件数記録なし",
    "candidateCount": "複数候補から1件を選定（総数記録なし）",
    "model": "Qwen2.5-3B-Instruct / ローカル実行",
    "temperature": "制作ログに記録なし",
    "seed": "制作ログに記録なし",
    "generatedAt": "公開前夜（詳細時刻の記録なし）",
    "exclusions": "個別メッセージ、授業チャンネル、送信されなかった本文",
    "edited": "本文の編集なし"
  },
  "scenes": [
    {
      "id": "current_notice",
      "title": "CURRENT｜学園祭の展示ホール",
      "chapter": "CURRENT",
      "modeIndex": 9,
      "steps": [
        {
          "id": "current_notice_001",
          "sceneId": "current_notice",
          "type": "narration",
          "speaker": "narrator",
          "text": "人の声とゲームの効果音が重なる通路に、小さなブースがある。\n端末の前には椅子が一脚。横には青りんごが一つ置かれている。"
        },
        {
          "id": "current_notice_002",
          "sceneId": "current_notice",
          "type": "narration",
          "speaker": "narrator",
          "text": "壁の制作者欄には、三人の表示名が同じ大きさで並んでいた。"
        },
        {
          "id": "current_notice_003",
          "sceneId": "current_notice",
          "type": "ui",
          "text": "MIZUHA\nAMANE\nSAKUYA"
        },
        {
          "id": "current_notice_004",
          "sceneId": "current_notice",
          "type": "narration",
          "speaker": "narrator",
          "text": "プレイヤーが椅子へ座る。\n青りんごと同じ色のSTARTボタンの下に、短い案内がある。"
        },
        {
          "id": "current_notice_005",
          "sceneId": "current_notice",
          "type": "narration",
          "speaker": "narrator",
          "text": "この端末は、進行と選んだ項目を会期中の件数として記録します。\n入力した文章は次の人へ表示せず、サーバーへ送りません。"
        },
        {
          "id": "current_notice_006",
          "sceneId": "current_notice",
          "type": "ui",
          "text": "記録について詳しく見る"
        },
        {
          "id": "current_notice_007",
          "sceneId": "current_notice",
          "type": "ui",
          "text": "START"
        }
      ],
      "nextSceneId": "opening_empty_seat"
    },
    {
      "id": "opening_empty_seat",
      "title": "OPENING｜三か月前／10:21",
      "chapter": "OPENING",
      "modeIndex": 9,
      "steps": [
        {
          "id": "opening_empty_seat_001",
          "sceneId": "opening_empty_seat",
          "type": "narration",
          "speaker": "narrator",
          "text": "大学の共同作業室には、長机が一つと椅子が三脚あった。"
        },
        {
          "id": "opening_empty_seat_002",
          "sceneId": "opening_empty_seat",
          "type": "narration",
          "speaker": "narrator",
          "text": "ミズハは保温ボトルと紙コップを三つ持ってきた。\nアマネは電源タップ、予備のケーブル、消毒用シートを机へ並べた。"
        },
        {
          "id": "opening_empty_seat_003",
          "sceneId": "opening_empty_seat",
          "type": "narration",
          "speaker": "narrator",
          "text": "真ん中の椅子には、誰も座っていない。"
        },
        {
          "id": "opening_empty_seat_004",
          "sceneId": "opening_empty_seat",
          "type": "chat",
          "time": "10:08",
          "speaker": "amane",
          "speakerLabel": "AMANE",
          "text": "先に部屋、入ってる。"
        },
        {
          "id": "opening_empty_seat_005",
          "sceneId": "opening_empty_seat",
          "type": "chat",
          "time": "10:09",
          "speaker": "mizuha",
          "speakerLabel": "MIZUHA",
          "text": "迷われたなら迎えに行きますわ。"
        },
        {
          "id": "opening_empty_seat_006",
          "sceneId": "opening_empty_seat",
          "type": "chat",
          "time": "10:21",
          "speaker": "amane",
          "speakerLabel": "AMANE",
          "text": "どこにいる？"
        },
        {
          "id": "opening_empty_seat_007",
          "sceneId": "opening_empty_seat",
          "type": "narration",
          "speaker": "narrator",
          "text": "短い文の横に未読の印が残った。"
        },
        {
          "id": "opening_empty_seat_008",
          "sceneId": "opening_empty_seat",
          "type": "narration",
          "speaker": "narrator",
          "text": "ミズハは紙コップへ茶を注いだ。\n二つではなく、三つ全部に注いだ。"
        },
        {
          "id": "opening_empty_seat_009",
          "sceneId": "opening_empty_seat",
          "type": "dialogue",
          "speaker": "amane",
          "text": "まだ、寝坊でいい？"
        },
        {
          "id": "opening_empty_seat_010",
          "sceneId": "opening_empty_seat",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "十一時までは。"
        },
        {
          "id": "opening_empty_seat_011",
          "sceneId": "opening_empty_seat",
          "type": "dialogue",
          "speaker": "amane",
          "text": "十時集合。"
        },
        {
          "id": "opening_empty_seat_012",
          "sceneId": "opening_empty_seat",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "存じていますわ。"
        },
        {
          "id": "opening_empty_seat_013",
          "sceneId": "opening_empty_seat",
          "type": "narration",
          "speaker": "narrator",
          "text": "廊下で足音がするたび、二人とも扉を見た。\n同じ階の利用者が通り過ぎ、コピー機が動き、別の部屋の扉が閉まった。"
        },
        {
          "id": "opening_empty_seat_014",
          "sceneId": "opening_empty_seat",
          "type": "narration",
          "speaker": "narrator",
          "text": "真ん中の紙コップから、先に湯気が見えなくなった。"
        },
        {
          "id": "opening_empty_seat_015",
          "sceneId": "opening_empty_seat",
          "type": "narration",
          "speaker": "narrator",
          "text": "画面は一年前へ戻る。"
        }
      ],
      "nextSceneId": "prologue_online_circle"
    },
    {
      "id": "prologue_online_circle",
      "title": "PROLOGUE｜文字だけだった三人",
      "chapter": "PROLOGUE",
      "modeIndex": 0,
      "steps": [
        {
          "id": "prologue_online_circle_001",
          "sceneId": "prologue_online_circle",
          "type": "narration",
          "speaker": "narrator",
          "text": "三人が通う大学では、授業の大半がブラウザの中にあった。\n別々の町から講義ページを開き、授業が終わればタブを閉じる。"
        },
        {
          "id": "prologue_online_circle_002",
          "sceneId": "prologue_online_circle",
          "type": "narration",
          "speaker": "narrator",
          "text": "大学のSlackには、学生が自由に作ったサークルが並んでいた。\nその一つが「惑星の放課後」だった。"
        },
        {
          "id": "prologue_online_circle_003",
          "sceneId": "prologue_online_circle",
          "type": "narration",
          "speaker": "narrator",
          "text": "ある授業のあと、雑談チャンネルの一行から始まった。"
        },
        {
          "id": "prologue_online_circle_004",
          "sceneId": "prologue_online_circle",
          "type": "narration",
          "speaker": "narrator",
          "text": "『人間は、地球を変える側にだけいるのだろうか。』"
        },
        {
          "id": "prologue_online_circle_005",
          "sceneId": "prologue_online_circle",
          "type": "narration",
          "speaker": "narrator",
          "text": "岩、池、屋上菜園、夜の地球。返信が続き、三日目に専用チャンネルができた。\n説明欄には『まだ名前のない変化を見つけて、持ち寄る場所』とあった。\n投稿する時間はばらばらで、一つの話題が何日も続いた。"
        }
      ],
      "nextSceneId": "prologue_basil"
    },
    {
      "id": "prologue_basil",
      "title": "PROLOGUE｜バジルの投稿",
      "chapter": "PROLOGUE",
      "modeIndex": 0,
      "steps": [
        {
          "id": "prologue_basil_001",
          "sceneId": "prologue_basil",
          "type": "narration",
          "speaker": "narrator",
          "text": "ミズハ、アマネ、サクヤが初めて同じスレッドに揃ったのは、園芸売り場の裏で撮られたバジルの写真だった。"
        },
        {
          "id": "prologue_basil_002",
          "sceneId": "prologue_basil",
          "type": "narration",
          "speaker": "narrator",
          "text": "値下げ札のついた鉢が、日陰で葉を垂らしている。\n写真の端には黄色い散水ホースと、土で汚れたスニーカーのつま先が写っていた。"
        },
        {
          "id": "prologue_basil_003",
          "sceneId": "prologue_basil",
          "type": "chat",
          "time": "18:12",
          "speaker": "mizuha",
          "speakerLabel": "MIZUHA",
          "text": "本日の売り場、三十六度ですの。\n土が湿っていても、葉から出る水へ根が追いつかないことがありますわ。"
        },
        {
          "id": "prologue_basil_004",
          "sceneId": "prologue_basil",
          "type": "chat",
          "time": "18:18",
          "speaker": "amane",
          "speakerLabel": "AMANE",
          "text": "店内に入れないの。"
        },
        {
          "id": "prologue_basil_005",
          "sceneId": "prologue_basil",
          "type": "chat",
          "time": "18:24",
          "speaker": "mizuha",
          "speakerLabel": "MIZUHA",
          "text": "売り場をすべて店内へ移せたら、私の時給では足りませんわね。"
        },
        {
          "id": "prologue_basil_006",
          "sceneId": "prologue_basil",
          "type": "narration",
          "speaker": "narrator",
          "text": "日付が変わるころ、見覚えのない花壇の写真が一枚増えた。\n花ではなく、日焼けして文字の薄くなった「水やり当番」の札へ焦点が合っていた。"
        },
        {
          "id": "prologue_basil_007",
          "sceneId": "prologue_basil",
          "type": "chat",
          "time": "00:31",
          "speaker": "sakuya",
          "speakerLabel": "SAKUYA",
          "text": "名前だけ残ってた。\n花はない。\n分からない。\n花壇をやめたのかもしれない。"
        },
        {
          "id": "prologue_basil_008",
          "sceneId": "prologue_basil",
          "type": "chat",
          "time": "00:51",
          "speaker": "mizuha",
          "speakerLabel": "MIZUHA",
          "text": "分からないなら、花がないところまでが今日の記録ですわね。"
        },
        {
          "id": "prologue_basil_009",
          "sceneId": "prologue_basil",
          "type": "narration",
          "speaker": "narrator",
          "text": "翌日の昼、葉を少し持ち上げたバジルの写真が届いた。\nアマネが、最寄りの観測所で公開された気温を探した。"
        },
        {
          "id": "prologue_basil_010",
          "sceneId": "prologue_basil",
          "type": "record",
          "recordType": "LOCAL_SOURCE",
          "text": "園芸売り場の温度計：36 ℃ / LOCAL SOURCE\n最寄り観測所の気温：観測所名と観測時刻を表示 / SOURCE"
        }
      ],
      "nextSceneId": "choice_observation_order"
    },
    {
      "id": "choice_observation_order",
      "title": "プレイヤーの小さな選択｜どちらから見る？",
      "chapter": "PROLOGUE",
      "modeIndex": 0,
      "choice": {
        "id": "observation_order",
        "prompt": "どちらから見る？",
        "trackedByEves": false,
        "options": [
          {
            "value": "LOCAL_FIRST",
            "next": "first_meeting_promise"
          },
          {
            "value": "STATION_FIRST",
            "next": "first_meeting_promise"
          }
        ]
      },
      "steps": [
        {
          "id": "choice_observation_order_001",
          "sceneId": "choice_observation_order",
          "type": "choice",
          "choiceId": "observation_order",
          "prompt": "どちらから見る？",
          "trackedByEves": false,
          "options": [
            {
              "value": "LOCAL_FIRST",
              "next": "first_meeting_promise",
              "label": "売り場の温度計から見る / LOCAL FIRST"
            },
            {
              "value": "STATION_FIRST",
              "next": "first_meeting_promise",
              "label": "最寄り観測所の記録から見る / STATION FIRST"
            }
          ]
        },
        {
          "id": "choice_observation_order_002",
          "sceneId": "choice_observation_order",
          "type": "narration",
          "speaker": "narrator",
          "text": "選んだあと、もう一方も開く。\n二つの値は同じ線へつながらず、開いた順番だけが操作記録に残る。"
        },
        {
          "id": "choice_observation_order_003",
          "sceneId": "choice_observation_order",
          "type": "narration",
          "speaker": "narrator",
          "text": "三人はそれから半年、顔を見ず、声も聞かず、文字だけで話した。\nハドルを開こうという話は二度出た。二度とも、誰も開始ボタンを押さなかった。"
        }
      ],
      "nextSceneId": "first_meeting_promise"
    },
    {
      "id": "first_meeting_promise",
      "title": "00:08｜初めて会う約束",
      "chapter": "PROLOGUE",
      "modeIndex": 0,
      "steps": [
        {
          "id": "first_meeting_promise_001",
          "sceneId": "first_meeting_promise",
          "type": "narration",
          "speaker": "narrator",
          "text": "学園祭当日の午前零時を過ぎたころ、サクヤが書いた。"
        },
        {
          "id": "first_meeting_promise_002",
          "sceneId": "first_meeting_promise",
          "type": "chat",
          "time": "00:08",
          "speaker": "sakuya",
          "speakerLabel": "SAKUYA",
          "text": "今日、会わない？"
        },
        {
          "id": "first_meeting_promise_003",
          "sceneId": "first_meeting_promise",
          "type": "chat",
          "time": "00:08",
          "speaker": "amane",
          "speakerLabel": "AMANE",
          "text": "今日？"
        },
        {
          "id": "first_meeting_promise_004",
          "sceneId": "first_meeting_promise",
          "type": "chat",
          "time": "00:09",
          "speaker": "mizuha",
          "speakerLabel": "MIZUHA",
          "text": "ずいぶん急ですのね。"
        },
        {
          "id": "first_meeting_promise_005",
          "sceneId": "first_meeting_promise",
          "type": "chat",
          "time": "00:09",
          "speaker": "sakuya",
          "speakerLabel": "SAKUYA",
          "text": "学園祭。\n三人で行きたい。"
        },
        {
          "id": "first_meeting_promise_006",
          "sceneId": "first_meeting_promise",
          "type": "narration",
          "speaker": "narrator",
          "text": "三人の「入力中」が、順番に出ては消えた。"
        },
        {
          "id": "first_meeting_promise_007",
          "sceneId": "first_meeting_promise",
          "type": "chat",
          "time": "00:11",
          "speaker": "mizuha",
          "speakerLabel": "MIZUHA",
          "text": "会場のどこで待ち合わせますの？"
        },
        {
          "id": "first_meeting_promise_008",
          "sceneId": "first_meeting_promise",
          "type": "chat",
          "time": "00:11",
          "speaker": "amane",
          "speakerLabel": "AMANE",
          "text": "入口、たぶん何個もある。\n人も多いよ。"
        },
        {
          "id": "first_meeting_promise_009",
          "sceneId": "first_meeting_promise",
          "type": "chat",
          "time": "00:12",
          "speaker": "mizuha",
          "speakerLabel": "MIZUHA",
          "text": "そもそも、私たちはお互いの顔が分かりませんわ。"
        },
        {
          "id": "first_meeting_promise_010",
          "sceneId": "first_meeting_promise",
          "type": "chat",
          "time": "00:12",
          "speaker": "sakuya",
          "speakerLabel": "SAKUYA",
          "text": "目印、青りんごにしよ。"
        },
        {
          "id": "first_meeting_promise_011",
          "sceneId": "first_meeting_promise",
          "type": "chat",
          "time": "00:13",
          "speaker": "amane",
          "speakerLabel": "AMANE",
          "text": "なんで青りんご。"
        },
        {
          "id": "first_meeting_promise_012",
          "sceneId": "first_meeting_promise",
          "type": "chat",
          "time": "00:13",
          "speaker": "sakuya",
          "speakerLabel": "SAKUYA",
          "text": "丸いし、青いし、地球っぽいから。"
        },
        {
          "id": "first_meeting_promise_013",
          "sceneId": "first_meeting_promise",
          "type": "chat",
          "time": "00:14",
          "speaker": "amane",
          "speakerLabel": "AMANE",
          "text": "名前は青だけど、緑じゃん。"
        },
        {
          "id": "first_meeting_promise_014",
          "sceneId": "first_meeting_promise",
          "type": "chat",
          "time": "00:14",
          "speaker": "sakuya",
          "speakerLabel": "SAKUYA",
          "text": "そこもいい。"
        },
        {
          "id": "first_meeting_promise_015",
          "sceneId": "first_meeting_promise",
          "type": "chat",
          "time": "00:16",
          "speaker": "mizuha",
          "speakerLabel": "MIZUHA",
          "text": "青りんごを持って、十時に中央入口。"
        },
        {
          "id": "first_meeting_promise_016",
          "sceneId": "first_meeting_promise",
          "type": "chat",
          "time": "00:16",
          "speaker": "amane",
          "speakerLabel": "AMANE",
          "text": "じゃあ、今日。"
        },
        {
          "id": "first_meeting_promise_017",
          "sceneId": "first_meeting_promise",
          "type": "chat",
          "time": "00:17",
          "speaker": "sakuya",
          "speakerLabel": "SAKUYA",
          "text": "うん。今日、はじめまして。"
        }
      ],
      "nextSceneId": "first_meeting_hall"
    },
    {
      "id": "first_meeting_hall",
      "title": "09:48｜海沿いの展示場・中央入口",
      "chapter": "PROLOGUE",
      "modeIndex": 0,
      "steps": [
        {
          "id": "first_meeting_hall_001",
          "sceneId": "first_meeting_hall",
          "type": "narration",
          "speaker": "narrator",
          "text": "駅から続く歩道橋は、朝から人が途切れなかった。\n段ボール箱を運ぶ学生と、首から入場証を下げた人が同じ方向へ歩いていく。"
        },
        {
          "id": "first_meeting_hall_002",
          "sceneId": "first_meeting_hall",
          "type": "narration",
          "speaker": "narrator",
          "text": "入口前の列は、ガラス張りの建物に沿って折れ曲がっていた。\n扉が開くたび、冷房の風と館内放送が外へ漏れた。"
        },
        {
          "id": "first_meeting_hall_003",
          "sceneId": "first_meeting_hall",
          "type": "narration",
          "speaker": "narrator",
          "text": "ミズハは人の流れから半歩離れ、胸の前で青りんごを持っていた。"
        },
        {
          "id": "first_meeting_hall_004",
          "sceneId": "first_meeting_hall",
          "type": "chat",
          "time": "09:54",
          "speaker": "amane",
          "speakerLabel": "AMANE",
          "text": "中央入口の右。\n青りんご持ってる？"
        },
        {
          "id": "first_meeting_hall_005",
          "sceneId": "first_meeting_hall",
          "type": "chat",
          "time": "09:54",
          "speaker": "mizuha",
          "speakerLabel": "MIZUHA",
          "text": "持っていますわ。"
        },
        {
          "id": "first_meeting_hall_006",
          "sceneId": "first_meeting_hall",
          "type": "chat",
          "time": "09:55",
          "speaker": "amane",
          "speakerLabel": "AMANE",
          "text": "たぶん、見つけた。"
        },
        {
          "id": "first_meeting_hall_007",
          "sceneId": "first_meeting_hall",
          "type": "narration",
          "speaker": "narrator",
          "text": "ミズハがスマートフォンから顔を上げた。\n黒いシャツの少女と目が合う。"
        },
        {
          "id": "first_meeting_hall_008",
          "sceneId": "first_meeting_hall",
          "type": "narration",
          "speaker": "narrator",
          "text": "二人とも、すぐには声を出さなかった。\nSlackなら、返事を考えていることを「入力中」が教えてくれる。\n画面の外には、その表示がなかった。"
        },
        {
          "id": "first_meeting_hall_009",
          "sceneId": "first_meeting_hall",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "アマネ、で合っていますの？"
        },
        {
          "id": "first_meeting_hall_010",
          "sceneId": "first_meeting_hall",
          "type": "dialogue",
          "speaker": "amane",
          "text": "うん。ミズハさん、だよね。"
        },
        {
          "id": "first_meeting_hall_011",
          "sceneId": "first_meeting_hall",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "毎日文字を交わしていましたのに、画面の外だとさん付けになるんですのね。"
        },
        {
          "id": "first_meeting_hall_012",
          "sceneId": "first_meeting_hall",
          "type": "dialogue",
          "speaker": "amane",
          "text": "そっちも。"
        },
        {
          "id": "first_meeting_hall_013",
          "sceneId": "first_meeting_hall",
          "type": "narration",
          "speaker": "narrator",
          "text": "サクヤが着いたのは十分後だった。\n二人の前まで来ても名乗らず、青りんごを一度指した。"
        },
        {
          "id": "first_meeting_hall_014",
          "sceneId": "first_meeting_hall",
          "type": "dialogue",
          "speaker": "sakuya",
          "text": "本物だった。"
        },
        {
          "id": "first_meeting_hall_015",
          "sceneId": "first_meeting_hall",
          "type": "dialogue",
          "speaker": "amane",
          "text": "提案した人がそれ言う？"
        },
        {
          "id": "first_meeting_hall_016",
          "sceneId": "first_meeting_hall",
          "type": "dialogue",
          "speaker": "sakuya",
          "text": "写真撮っていい？"
        },
        {
          "id": "first_meeting_hall_017",
          "sceneId": "first_meeting_hall",
          "type": "narration",
          "speaker": "narrator",
          "text": "三人の手と青りんごだけが、最初の一枚に写った。"
        }
      ],
      "nextSceneId": "festival_walk"
    },
    {
      "id": "festival_walk",
      "title": "17:06｜展示ホールをつなぐ連絡通路",
      "chapter": "PROLOGUE",
      "modeIndex": 0,
      "steps": [
        {
          "id": "festival_walk_001",
          "sceneId": "festival_walk",
          "type": "narration",
          "speaker": "narrator",
          "text": "三人は一日、学生のブースを歩いた。\n風向きで光の粒が流れる地図。自転車をこぐと町の電力が変わる模型。来場者の短い文章が天井へ投影される展示。"
        },
        {
          "id": "festival_walk_002",
          "sceneId": "festival_walk",
          "type": "narration",
          "speaker": "narrator",
          "text": "見終えたものは同じでも、三人のメモは違った。"
        },
        {
          "id": "festival_walk_003",
          "sceneId": "festival_walk",
          "type": "narration",
          "speaker": "narrator",
          "text": "ミズハのノートには、生き物の名前と表示されなかった条件。\nアマネのスマートフォンには、操作に迷った画面と電源の位置。\nサクヤの写真には、作品より先に説明札を読む人と、出口で振り返る人が写っていた。"
        },
        {
          "id": "festival_walk_004",
          "sceneId": "festival_walk",
          "type": "dialogue",
          "speaker": "sakuya",
          "text": "次は、見る側じゃなくて出す側やらない？"
        },
        {
          "id": "festival_walk_005",
          "sceneId": "festival_walk",
          "type": "dialogue",
          "speaker": "amane",
          "text": "何を出すの。"
        },
        {
          "id": "festival_walk_006",
          "sceneId": "festival_walk",
          "type": "dialogue",
          "speaker": "sakuya",
          "text": "まだ分かんない。\nでも、三人とも拾ってるところが違ったから。"
        },
        {
          "id": "festival_walk_007",
          "sceneId": "festival_walk",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "では、今日気になったものを一つの作品へ持ち寄ってみます？"
        },
        {
          "id": "festival_walk_008",
          "sceneId": "festival_walk",
          "type": "narration",
          "speaker": "narrator",
          "text": "アマネは返事の代わりに、次の学園祭の出展案内を保存した。"
        },
        {
          "id": "festival_walk_009",
          "sceneId": "festival_walk",
          "type": "narration",
          "speaker": "narrator",
          "text": "三人は同じ改札を通り、別々のホームへ分かれた。\n帰宅すると、三人だけのチャンネルと、空の共有フォルダができていた。"
        },
        {
          "id": "festival_walk_010",
          "sceneId": "festival_walk",
          "type": "narration",
          "speaker": "narrator",
          "text": "GAIA SENSEWARE"
        }
      ],
      "nextSceneId": "production_year"
    },
    {
      "id": "production_year",
      "title": "九か月間｜三人で作ったもの",
      "chapter": "PRODUCTION",
      "modeIndex": 5,
      "steps": [
        {
          "id": "production_year_001",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "三人は、地球について公開されている記録を、触って確かめられる画面へ変えることにした。"
        },
        {
          "id": "production_year_002",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "ミズハが観測条件を調べる。\nアマネがデータと操作をつなぐ。\nサクヤが文章と表示順を決める。"
        },
        {
          "id": "production_year_003",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "それで綺麗に分担できたのは、最初の一週間だけだった。"
        },
        {
          "id": "production_year_004",
          "sceneId": "production_year",
          "type": "transition",
          "text": "白と赤"
        },
        {
          "id": "production_year_005",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "夜間光の画面を作った夜、アマネの試作では都市の白い光を、排出量の赤い格子が覆っていた。"
        },
        {
          "id": "production_year_006",
          "sceneId": "production_year",
          "type": "chat",
          "time": "00:16",
          "speaker": "sakuya",
          "speakerLabel": "SAKUYA",
          "text": "白、消さないで。"
        },
        {
          "id": "production_year_007",
          "sceneId": "production_year",
          "type": "chat",
          "time": "00:18",
          "speaker": "amane",
          "speakerLabel": "AMANE",
          "text": "排出を見せる画面だから。"
        },
        {
          "id": "production_year_008",
          "sceneId": "production_year",
          "type": "chat",
          "time": "00:22",
          "speaker": "sakuya",
          "speakerLabel": "SAKUYA",
          "text": "そこに住んでる人の灯りでもある。"
        },
        {
          "id": "production_year_009",
          "sceneId": "production_year",
          "type": "chat",
          "time": "00:31",
          "speaker": "mizuha",
          "speakerLabel": "MIZUHA",
          "text": "二つの層を切り替えるのはどうかしら。"
        },
        {
          "id": "production_year_010",
          "sceneId": "production_year",
          "type": "chat",
          "time": "00:38",
          "speaker": "sakuya",
          "speakerLabel": "SAKUYA",
          "text": "比較のために、白を悪い色にしないで。"
        },
        {
          "id": "production_year_011",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "アマネの返信は止まった。\n翌朝、白と赤は別々に表示され、重ねても両方の数値が開くようになっていた。"
        },
        {
          "id": "production_year_012",
          "sceneId": "production_year",
          "type": "chat",
          "time": "06:54",
          "speaker": "amane",
          "speakerLabel": "AMANE",
          "text": "これなら。"
        },
        {
          "id": "production_year_013",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "サクヤは昼過ぎに目のリアクションを付けた。"
        },
        {
          "id": "production_year_014",
          "sceneId": "production_year",
          "type": "transition",
          "text": "サクヤが消した一行"
        },
        {
          "id": "production_year_015",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "夏、森と降水量を重ねる画面で、サクヤが説明を短くした。"
        },
        {
          "id": "production_year_016",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "変更前：森林と雨の分布には重なる場所がある。これだけでは因果関係を断定できない。\n変更後：森が雨を呼ぶ。"
        },
        {
          "id": "production_year_017",
          "sceneId": "production_year",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "これは戻しますわ。"
        },
        {
          "id": "production_year_018",
          "sceneId": "production_year",
          "type": "dialogue",
          "speaker": "sakuya",
          "text": "長いと読まれない。"
        },
        {
          "id": "production_year_019",
          "sceneId": "production_year",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "短ければ、違っていてもよろしいの？"
        },
        {
          "id": "production_year_020",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "サクヤから返事が来なくなった。"
        },
        {
          "id": "production_year_021",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "アマネは変更後の一行を公開用の画面から外した。\n元の説明を短くし、「森と雨が重なる場所があります。原因はこの画面だけでは決められません」へ直した。"
        },
        {
          "id": "production_year_022",
          "sceneId": "production_year",
          "type": "chat",
          "time": "08:12",
          "speaker": "sakuya",
          "speakerLabel": "SAKUYA",
          "text": "短くしすぎた。"
        },
        {
          "id": "production_year_023",
          "sceneId": "production_year",
          "type": "chat",
          "time": "08:16",
          "speaker": "mizuha",
          "speakerLabel": "MIZUHA",
          "text": "二日お待ちしましたわ。"
        },
        {
          "id": "production_year_024",
          "sceneId": "production_year",
          "type": "chat",
          "time": "08:29",
          "speaker": "sakuya",
          "speakerLabel": "SAKUYA",
          "text": "ごめん。"
        },
        {
          "id": "production_year_025",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "ミズハは昼休みに、修正後の一行へ確認の印を付けた。"
        },
        {
          "id": "production_year_026",
          "sceneId": "production_year",
          "type": "transition",
          "text": "年越しのハドル"
        },
        {
          "id": "production_year_027",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "年末、三人は初めてハドルを開いた。\n通話開始から十一秒、誰も話さなかった。"
        },
        {
          "id": "production_year_028",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "アマネの扇風機。\nミズハが湯を沸かす音。\nサクヤの窓から聞こえる踏切。"
        },
        {
          "id": "production_year_029",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "最初に話したのはミズハだった。"
        },
        {
          "id": "production_year_030",
          "sceneId": "production_year",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "聞こえています？"
        },
        {
          "id": "production_year_031",
          "sceneId": "production_year",
          "type": "dialogue",
          "speaker": "amane",
          "text": "聞こえてる。"
        },
        {
          "id": "production_year_032",
          "sceneId": "production_year",
          "type": "dialogue",
          "speaker": "sakuya",
          "text": "うちも。"
        },
        {
          "id": "production_year_033",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "それだけ確かめると、三人は画面共有へ戻った。\n音の確認用に録音を動かしていたため、最初の無言もファイルへ残った。\nサクヤは冒頭の十一秒を切らず、`first_huddle.wav`として保存した。"
        },
        {
          "id": "production_year_034",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "午前零時を過ぎたとき、アマネは冷蔵庫から期限が昨日までの食パンを出した。\nミズハは売り場でもらった小さなミカンを三つ食べた。\nサクヤはカップ麺の蓋を押さえるため、読みかけの文庫本を載せていた。"
        },
        {
          "id": "production_year_035",
          "sceneId": "production_year",
          "type": "transition",
          "text": "公開する約束"
        },
        {
          "id": "production_year_036",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "次の学園祭へ出展申請を出す前に、アマネが一枚の確認画面を作った。"
        },
        {
          "id": "production_year_037",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "公開同意 / PUBLICATION AGREEMENT\n学園祭公開版 1.0 / FESTIVAL BUILD 1.0"
        },
        {
          "id": "production_year_038",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "- この版に保存した文章、画像、音、表示名を公開してよい。\n- 誰が作った記録かを変えない。\n- 生成した文は本人の文と分け、公開対象に選んだ制作投稿だけを生成へ使う。"
        },
        {
          "id": "production_year_039",
          "sceneId": "production_year",
          "type": "dialogue",
          "speaker": "amane",
          "text": "全員が確認するまで申請しない。"
        },
        {
          "id": "production_year_040",
          "sceneId": "production_year",
          "type": "narration",
          "speaker": "narrator",
          "text": "三人は自分の表示名の横に、公開可の印を付けた。\n出展申請へは本名と学生番号を記入し、作品の制作者欄には表示名を使った。\nサクヤは制作者名の位置を二ミリだけ右へ動かした。"
        },
        {
          "id": "production_year_041",
          "sceneId": "production_year",
          "type": "dialogue",
          "speaker": "amane",
          "text": "今そこ？"
        },
        {
          "id": "production_year_042",
          "sceneId": "production_year",
          "type": "dialogue",
          "speaker": "sakuya",
          "text": "三人の真ん中、ずれてた。"
        }
      ],
      "nextSceneId": "absence"
    },
    {
      "id": "absence",
      "title": "三か月前／来なかった日",
      "chapter": "ABSENCE",
      "modeIndex": 2,
      "steps": [
        {
          "id": "absence_001",
          "sceneId": "absence",
          "type": "narration",
          "speaker": "narrator",
          "text": "画面は、最初の共同作業室へ戻る。"
        },
        {
          "id": "absence_002",
          "sceneId": "absence",
          "type": "narration",
          "speaker": "narrator",
          "text": "十時二十七分、アマネが制作チャンネルを開くと、深夜の投稿が二件あった。"
        },
        {
          "id": "absence_003",
          "sceneId": "absence",
          "type": "chat",
          "time": "02:13",
          "speaker": "sakuya",
          "speakerLabel": "SAKUYA",
          "text": "データだけ先に上げた。\n森と雨が重なっても、「森が雨を降らせた」にはしないでね。"
        },
        {
          "id": "absence_004",
          "sceneId": "absence",
          "type": "chat",
          "time": "02:14",
          "speaker": "sakuya",
          "speakerLabel": "SAKUYA",
          "text": "もし地球の声が聞こえたと思ったら、すぐに意味を決めるんじゃなくて――"
        },
        {
          "id": "absence_005",
          "sceneId": "absence",
          "type": "narration",
          "speaker": "narrator",
          "text": "十一時を過ぎても、サクヤの席は空いていた。"
        },
        {
          "id": "absence_006",
          "sceneId": "absence",
          "type": "dialogue",
          "speaker": "amane",
          "text": "昨日まで、普通に直してる。"
        },
        {
          "id": "absence_007",
          "sceneId": "absence",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "寝坊なら、あとで文句を言えば済みますわ。"
        },
        {
          "id": "absence_008",
          "sceneId": "absence",
          "type": "narration",
          "speaker": "narrator",
          "text": "昼を過ぎても未読の印は消えなかった。\n三時になると、ミズハは冷えた紙コップを流しへ運んだ。\nサクヤの分だけ、縁が乾いたままだった。"
        },
        {
          "id": "absence_009",
          "sceneId": "absence",
          "type": "dialogue",
          "speaker": "amane",
          "text": "それ、捨てるの。"
        },
        {
          "id": "absence_010",
          "sceneId": "absence",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "お茶ですもの。"
        },
        {
          "id": "absence_011",
          "sceneId": "absence",
          "type": "narration",
          "speaker": "narrator",
          "text": "アマネは自分の紙コップを先に空けた。\nそれからサクヤの紙コップを取り、冷えた茶を二口で飲んだ。"
        }
      ],
      "nextSceneId": "search"
    },
    {
      "id": "search",
      "title": "その日から二週間｜安否確認",
      "chapter": "SEARCH",
      "modeIndex": 2,
      "steps": [
        {
          "id": "search_001",
          "sceneId": "search",
          "type": "narration",
          "speaker": "narrator",
          "text": "二人はSlack、大学メール、共有されていた連絡先へ送り、通話も試した。\nサークルの世話役と共通の履修者へ頼み、大学の学生支援窓口には本名、学生番号、最後の連絡時刻を伝えた。"
        },
        {
          "id": "search_002",
          "sceneId": "search",
          "type": "narration",
          "speaker": "narrator",
          "text": "警察相談窓口にも同じ情報を提出した。\n住所と家族の連絡先は知らず、大学から確認結果を聞くこともできなかった。"
        },
        {
          "id": "search_003",
          "sceneId": "search",
          "type": "narration",
          "speaker": "narrator",
          "text": "分かったのは、連絡が届かないことだけだった。"
        },
        {
          "id": "search_004",
          "sceneId": "search",
          "type": "narration",
          "speaker": "narrator",
          "text": "一週目、ミズハは朝と夜にSlackを開いた。\n二週目、アマネは未読の印を数えるのをやめた。"
        },
        {
          "id": "search_005",
          "sceneId": "search",
          "type": "narration",
          "speaker": "narrator",
          "text": "二人は共同作業室をもう一度予約した。\n椅子を二脚だけ出すか決められず、三脚とも机へ並べた。"
        },
        {
          "id": "search_006",
          "sceneId": "search",
          "type": "narration",
          "speaker": "narrator",
          "text": "学園祭へ出すのは、三人が公開を確認したFESTIVAL BUILD 1.0まで。\nサクヤ名義の文章は足さない。\nその後の変更は、ミズハとアマネの名前で履歴を残す。"
        },
        {
          "id": "search_007",
          "sceneId": "search",
          "type": "narration",
          "speaker": "narrator",
          "text": "二人はその規則を決めてから、作業を再開した。"
        }
      ],
      "nextSceneId": "festival_build"
    },
    {
      "id": "festival_build",
      "title": "CURRENT｜公開されたGAIA SENSEWARE",
      "chapter": "CURRENT",
      "modeIndex": 9,
      "steps": [
        {
          "id": "festival_build_001",
          "sceneId": "festival_build",
          "type": "narration",
          "speaker": "narrator",
          "text": "現在の展示ホールへ戻る。"
        },
        {
          "id": "festival_build_002",
          "sceneId": "festival_build",
          "type": "narration",
          "speaker": "narrator",
          "text": "学園祭公開版 1.0 / FESTIVAL BUILD 1.0：EDITORS 3 / PUBLICATION AGREED\n公開同意後の変更 / POST-BUILD CHANGES：EDITORS 2 / ATTRIBUTED SEPARATELY"
        }
      ],
      "nextSceneId": "gx_deep_time"
    },
    {
      "id": "gx_deep_time",
      "title": "GXモード｜太古の海",
      "chapter": "GX",
      "modeIndex": 0,
      "interaction": {
        "kind": "gx",
        "requiredGestures": 3
      },
      "steps": [
        {
          "id": "gx_deep_time_001",
          "sceneId": "gx_deep_time",
          "type": "narration",
          "speaker": "narrator",
          "text": "現在から四十六億年ぶんの目盛りが、画面の上を戻っていく。\n数字が止まると、岩と水だけの暗い海が開いた。"
        },
        {
          "id": "gx_deep_time_002",
          "sceneId": "gx_deep_time",
          "type": "interaction",
          "interaction": {
            "kind": "gx",
            "requiredGestures": 3
          },
          "text": "操作｜水面をなぞる"
        },
        {
          "id": "gx_deep_time_003",
          "sceneId": "gx_deep_time",
          "type": "narration",
          "speaker": "narrator",
          "text": "プレイヤーが指を動かした場所へ、青緑の点が残る。\n一度では足りない。何度かなぞるうちに点が増え、海の色が少しずつ変わる。"
        },
        {
          "id": "gx_deep_time_004",
          "sceneId": "gx_deep_time",
          "type": "narration",
          "speaker": "narrator",
          "text": "光合成で生じた酸素は、最初から空へたまったわけではなかった。\n海の中の鉄などと反応したあと、長い時間をかけて大気にも増えていった。"
        },
        {
          "id": "gx_deep_time_005",
          "sceneId": "gx_deep_time",
          "type": "record",
          "recordType": "DERIVED",
          "text": "計算・解釈 / DERIVED\n太古の記録を、触れられる時間軸へ並べた表示"
        },
        {
          "id": "gx_deep_time_006",
          "sceneId": "gx_deep_time",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "今の役割から、最初の目的を決めることはできませんわ。"
        },
        {
          "id": "gx_deep_time_007",
          "sceneId": "gx_deep_time",
          "type": "dialogue",
          "speaker": "amane",
          "text": "酸素を使う生き物のために、酸素を作ったわけじゃない。"
        },
        {
          "id": "gx_deep_time_008",
          "sceneId": "gx_deep_time",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "ええ。できたものが、あとから別の生き物にも使われましたの。"
        },
        {
          "id": "gx_deep_time_009",
          "sceneId": "gx_deep_time",
          "type": "narration",
          "speaker": "narrator",
          "text": "プレイヤーが指を離す。\n太古の海が一枚の地図へ縮み、現在の森林分布に切り替わる。"
        }
      ],
      "nextSceneId": "mode03_map"
    },
    {
      "id": "mode03_map",
      "title": "MODE 03｜地図モード／森の気候装置",
      "chapter": "MODE 03",
      "modeIndex": 2,
      "interaction": {
        "kind": "map03",
        "requiredLayers": [
          "forest",
          "rain",
          "overlay"
        ]
      },
      "steps": [
        {
          "id": "mode03_map_001",
          "sceneId": "mode03_map",
          "type": "ui",
          "text": "表示｜地図モード"
        },
        {
          "id": "mode03_map_002",
          "sceneId": "mode03_map",
          "type": "narration",
          "speaker": "narrator",
          "text": "プレイヤーが「森林」を開く。\n観測年と提供元を添えた森林分布が緑で表示される。"
        },
        {
          "id": "mode03_map_003",
          "sceneId": "mode03_map",
          "type": "narration",
          "speaker": "narrator",
          "text": "次に「降水量」を開く。\n同じ時期の降水量が青で表示される。"
        },
        {
          "id": "mode03_map_004",
          "sceneId": "mode03_map",
          "type": "interaction",
          "interaction": {
            "kind": "map03",
            "requiredLayers": [
              "forest",
              "rain",
              "overlay"
            ]
          },
          "text": "操作｜二つの記録を重ねる"
        },
        {
          "id": "mode03_map_005",
          "sceneId": "mode03_map",
          "type": "narration",
          "speaker": "narrator",
          "text": "森林分布へ降水量を重ねる。\n緑と青が重なる場所と、片方しか記録されていない場所がある。"
        },
        {
          "id": "mode03_map_006",
          "sceneId": "mode03_map",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "重なっている場所は見えます。ただ、この画面だけで原因と結果は決められませんわ。"
        },
        {
          "id": "mode03_map_007",
          "sceneId": "mode03_map",
          "type": "dialogue",
          "speaker": "amane",
          "text": "あの一行、残ってる。"
        },
        {
          "id": "mode03_map_008",
          "sceneId": "mode03_map",
          "type": "narration",
          "speaker": "narrator",
          "text": "制作履歴が開く。"
        },
        {
          "id": "mode03_map_009",
          "sceneId": "mode03_map",
          "type": "narration",
          "speaker": "narrator",
          "text": "削除 / REMOVED：森が雨を呼ぶ。\n復元 / RESTORED：森と雨が重なる場所があります。原因はこの画面だけでは決められません。\n編集履歴 / EDIT HISTORY：SAKUYA / MIZUHA / AMANE"
        },
        {
          "id": "mode03_map_010",
          "sceneId": "mode03_map",
          "type": "narration",
          "speaker": "narrator",
          "text": "サクヤが短くしすぎた文章と、三人で直した文章が並ぶ。"
        },
        {
          "id": "mode03_map_011",
          "sceneId": "mode03_map",
          "type": "narration",
          "speaker": "narrator",
          "text": "その下に、公開版へ入っていない二件の投稿があった。"
        },
        {
          "id": "mode03_map_012",
          "sceneId": "mode03_map",
          "type": "record",
          "recordType": "SOURCE",
          "text": "直前の投稿 / CONTEXT\n02:13  SAKUYA\nデータだけ先に上げた。\n森と雨が重なっても、「森が雨を降らせた」にはしないでね。"
        },
        {
          "id": "mode03_map_013",
          "sceneId": "mode03_map",
          "type": "record",
          "recordType": "SOURCE",
          "text": "最後の受信文 / SOURCE\n02:14  SAKUYA\nもし地球の声が聞こえたと思ったら、すぐに意味を決めるんじゃなくて――"
        },
        {
          "id": "mode03_map_014",
          "sceneId": "mode03_map",
          "type": "narration",
          "speaker": "narrator",
          "text": "このあとに届いたメッセージ：0"
        },
        {
          "id": "mode03_map_015",
          "sceneId": "mode03_map",
          "type": "narration",
          "speaker": "narrator",
          "text": "画面は二件を切り離さず、送られた順に残した。"
        }
      ],
      "nextSceneId": "mode07_abstract"
    },
    {
      "id": "mode07_abstract",
      "title": "MODE 07｜抽象モード／届いた時刻、開いた時刻",
      "chapter": "MODE 07",
      "modeIndex": 6,
      "interaction": {
        "kind": "abstract07",
        "requiredPoints": 1
      },
      "steps": [
        {
          "id": "mode07_abstract_001",
          "sceneId": "mode07_abstract",
          "type": "ui",
          "text": "表示｜地図モードから抽象モードへ"
        },
        {
          "id": "mode07_abstract_002",
          "sceneId": "mode07_abstract",
          "type": "narration",
          "speaker": "narrator",
          "text": "地図の震源と観測点だけが残り、町名と境界線が消える。\n震源が画面の中央へ移る。"
        },
        {
          "id": "mode07_abstract_003",
          "sceneId": "mode07_abstract",
          "type": "interaction",
          "interaction": {
            "kind": "abstract07",
            "requiredPoints": 1
          },
          "text": "操作｜観測点に触れる"
        },
        {
          "id": "mode07_abstract_004",
          "sceneId": "mode07_abstract",
          "type": "narration",
          "speaker": "narrator",
          "text": "プレイヤーが一つの観測点に触れる。\n震源から速い波と遅い波が輪になって広がり、触れた地点へ別々の時刻に届く。\n到着時刻と観測震度が、地名ではなく距離と時間の列として出る。"
        },
        {
          "id": "mode07_abstract_005",
          "sceneId": "mode07_abstract",
          "type": "dialogue",
          "speaker": "amane",
          "text": "起きた時刻と、届いた時刻と、気づいた時刻。全部別。"
        },
        {
          "id": "mode07_abstract_006",
          "sceneId": "mode07_abstract",
          "type": "narration",
          "speaker": "narrator",
          "text": "地震の時刻表示の横に、サクヤの投稿時刻が重なる。"
        },
        {
          "id": "mode07_abstract_007",
          "sceneId": "mode07_abstract",
          "type": "narration",
          "speaker": "narrator",
          "text": "受信時刻 / RECEIVED：02:14\n二人が開いた時刻 / OPENED：10:27"
        },
        {
          "id": "mode07_abstract_008",
          "sceneId": "mode07_abstract",
          "type": "narration",
          "speaker": "narrator",
          "text": "公開前夜の記録が始まる。"
        },
        {
          "id": "mode07_abstract_009",
          "sceneId": "mode07_abstract",
          "type": "narration",
          "speaker": "narrator",
          "text": "ミズハは、公開対象になっていたサクヤの投稿を使い、文末に続く候補を作っていた。"
        },
        {
          "id": "mode07_abstract_010",
          "sceneId": "mode07_abstract",
          "type": "record",
          "recordType": "SOURCE",
          "text": "観測記録 / SOURCE\n作者 / AUTHOR：SAKUYA"
        },
        {
          "id": "mode07_abstract_011",
          "sceneId": "mode07_abstract",
          "type": "narration",
          "speaker": "narrator",
          "text": "もし地球の声が聞こえたと思ったら、すぐに意味を決めるんじゃなくて――"
        },
        {
          "id": "mode07_abstract_012",
          "sceneId": "mode07_abstract",
          "type": "record",
          "recordType": "DERIVED",
          "text": "計算・解釈 / DERIVED\n生成部分 / GENERATED TEXT"
        },
        {
          "id": "mode07_abstract_013",
          "sceneId": "mode07_abstract",
          "type": "narration",
          "speaker": "narrator",
          "text": "「聞こえたつもりになってない？」って、三人で確かめたい。"
        },
        {
          "id": "mode07_abstract_014",
          "sceneId": "mode07_abstract",
          "type": "narration",
          "speaker": "narrator",
          "text": "生成元 / GENERATED FROM：公開対象として選定済みの制作チャンネル投稿からローカル生成\n生成実行 / GENERATED BY：MIZUHA\n選定責任 / SELECTED BY：MIZUHA\n本人の確認 / CONFIRMED BY SAKUYA：なし"
        },
        {
          "id": "mode07_abstract_015",
          "sceneId": "mode07_abstract",
          "type": "details",
          "text": "生成履歴を詳しく見る",
          "detailId": "mode07_generation_details"
        },
        {
          "id": "mode07_abstract_016",
          "sceneId": "mode07_abstract",
          "type": "narration",
          "speaker": "narrator",
          "text": "公開対象の制作投稿だけを参照。個別メッセージと授業チャンネルは対象外。\nローカル環境で複数候補を生成し、ミズハが一件を選んだ。本文の編集はしていない。"
        },
        {
          "id": "mode07_abstract_017",
          "sceneId": "mode07_abstract",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "似ておりますわね。"
        },
        {
          "id": "mode07_abstract_018",
          "sceneId": "mode07_abstract",
          "type": "dialogue",
          "speaker": "amane",
          "text": "だから、本人の続きには置けない。"
        },
        {
          "id": "mode07_abstract_019",
          "sceneId": "mode07_abstract",
          "type": "narration",
          "speaker": "narrator",
          "text": "アマネは候補文を公開画面から外した。"
        },
        {
          "id": "mode07_abstract_020",
          "sceneId": "mode07_abstract",
          "type": "narration",
          "speaker": "narrator",
          "text": "ミズハは何も言わず、表示を戻した。"
        },
        {
          "id": "mode07_abstract_021",
          "sceneId": "mode07_abstract",
          "type": "dialogue",
          "speaker": "amane",
          "text": "戻さないで。"
        },
        {
          "id": "mode07_abstract_022",
          "sceneId": "mode07_abstract",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "サクヤの文章とは書いていませんわ。"
        },
        {
          "id": "mode07_abstract_023",
          "sceneId": "mode07_abstract",
          "type": "dialogue",
          "speaker": "amane",
          "text": "ラベルを全部読む人ばかりじゃない。"
        },
        {
          "id": "mode07_abstract_024",
          "sceneId": "mode07_abstract",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "では、作らなかったことにしますの？"
        },
        {
          "id": "mode07_abstract_025",
          "sceneId": "mode07_abstract",
          "type": "dialogue",
          "speaker": "amane",
          "text": "作ったのはミズハでしょ。"
        },
        {
          "id": "mode07_abstract_026",
          "sceneId": "mode07_abstract",
          "type": "narration",
          "speaker": "narrator",
          "text": "ミズハの手がマウスから離れた。\n机の端に置いた水は、蓋を開けたまま減っていなかった。"
        },
        {
          "id": "mode07_abstract_027",
          "sceneId": "mode07_abstract",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "それは、ずるい言い方ですわ。"
        },
        {
          "id": "mode07_abstract_028",
          "sceneId": "mode07_abstract",
          "type": "dialogue",
          "speaker": "amane",
          "text": "分かってる。"
        },
        {
          "id": "mode07_abstract_029",
          "sceneId": "mode07_abstract",
          "type": "narration",
          "speaker": "narrator",
          "text": "アマネは青い文をもう一度非表示にした。\nファイルは消さず、名前を変えた。"
        },
        {
          "id": "mode07_abstract_030",
          "sceneId": "mode07_abstract",
          "type": "narration",
          "speaker": "narrator",
          "text": "SAKUYA_LAST_MESSAGE.txt\n　　　↓\nSAKUYA_STYLE_DERIVED_01.txt"
        },
        {
          "id": "mode07_abstract_031",
          "sceneId": "mode07_abstract",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "白い文だけ置いたら、私たちが何もしなかったように見えますわ。"
        },
        {
          "id": "mode07_abstract_032",
          "sceneId": "mode07_abstract",
          "type": "dialogue",
          "speaker": "amane",
          "text": "見せたら、サクヤが書いたと思う人が出る。"
        },
        {
          "id": "mode07_abstract_033",
          "sceneId": "mode07_abstract",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "私の名前を出します。"
        },
        {
          "id": "mode07_abstract_034",
          "sceneId": "mode07_abstract",
          "type": "dialogue",
          "speaker": "amane",
          "text": "だったら最初から、自分の文章として書けばよかった。"
        },
        {
          "id": "mode07_abstract_035",
          "sceneId": "mode07_abstract",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "自分の文章では、待っているふりができませんもの。"
        },
        {
          "id": "mode07_abstract_036",
          "sceneId": "mode07_abstract",
          "type": "narration",
          "speaker": "narrator",
          "text": "アマネは空になった缶コーヒーを振った。\n音はしなかった。"
        },
        {
          "id": "mode07_abstract_037",
          "sceneId": "mode07_abstract",
          "type": "dialogue",
          "speaker": "amane",
          "text": "私は、これをサクヤのいい台詞だと思われるのが嫌。"
        },
        {
          "id": "mode07_abstract_038",
          "sceneId": "mode07_abstract",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "私は、これを隠して、何も間違えなかった顔をするのが嫌ですわ。"
        },
        {
          "id": "mode07_abstract_039",
          "sceneId": "mode07_abstract",
          "type": "narration",
          "speaker": "narrator",
          "text": "二人とも表示には触れなかった。\n締切までの残り時間だけが、画面の右上で減った。"
        },
        {
          "id": "mode07_abstract_040",
          "sceneId": "mode07_abstract",
          "type": "narration",
          "speaker": "narrator",
          "text": "プレイヤーは白い原文と青い生成部分の情報欄を開く。\n青い文には分類、生成元、生成実行者、選定責任者、本人未確認が表示される。\n参照範囲、生成方法、モデル、時刻は「生成履歴を詳しく見る」から任意で開ける。"
        },
        {
          "id": "mode07_abstract_041",
          "sceneId": "mode07_abstract",
          "type": "narration",
          "speaker": "narrator",
          "text": "［この画面では選択肢を出さない］\n二つを見た記録だけが、MODE 10へ残る。"
        }
      ],
      "nextSceneId": "interlude_sea"
    },
    {
      "id": "interlude_sea",
      "title": "INTERLUDE｜十二分の海",
      "chapter": "INTERLUDE",
      "modeIndex": 6,
      "steps": [
        {
          "id": "interlude_sea_001",
          "sceneId": "interlude_sea",
          "type": "narration",
          "speaker": "narrator",
          "text": "公開前夜、二人は共同作業室を出た。\n建物の自動販売機で、アマネは無糖の缶コーヒーを買った。\nミズハはボタンを押す前に財布を見て、いちばん安い水に変えた。"
        },
        {
          "id": "interlude_sea_002",
          "sceneId": "interlude_sea",
          "type": "narration",
          "speaker": "narrator",
          "text": "坂を下ると海岸へ出た。\n終電まで時間はなく、アマネがスマートフォンのタイマーを十二分に設定した。"
        },
        {
          "id": "interlude_sea_003",
          "sceneId": "interlude_sea",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "海まで来て、十二分。"
        },
        {
          "id": "interlude_sea_004",
          "sceneId": "interlude_sea",
          "type": "dialogue",
          "speaker": "amane",
          "text": "往復入れたら、それ以上ない。"
        },
        {
          "id": "interlude_sea_005",
          "sceneId": "interlude_sea",
          "type": "narration",
          "speaker": "narrator",
          "text": "二人は護岸の低い段差へ座った。\nミズハの紙袋には、翌日のブースへ置くために買った青りんごが一つ入っていた。\n二人とも夕食を取っていなかった。"
        },
        {
          "id": "interlude_sea_006",
          "sceneId": "interlude_sea",
          "type": "dialogue",
          "speaker": "amane",
          "text": "丸かじりすれば。"
        },
        {
          "id": "interlude_sea_007",
          "sceneId": "interlude_sea",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "歯形まで展示へ持ち帰りたくありませんわ。"
        },
        {
          "id": "interlude_sea_008",
          "sceneId": "interlude_sea",
          "type": "narration",
          "speaker": "narrator",
          "text": "残り七分。\n二人は作品の話をしなかった。"
        },
        {
          "id": "interlude_sea_009",
          "sceneId": "interlude_sea",
          "type": "narration",
          "speaker": "narrator",
          "text": "アマネは缶の水滴を親指で拭いた。\nミズハは紙袋の口を二度折った。"
        },
        {
          "id": "interlude_sea_010",
          "sceneId": "interlude_sea",
          "type": "narration",
          "speaker": "narrator",
          "text": "残り三分で、ミズハが言った。"
        },
        {
          "id": "interlude_sea_011",
          "sceneId": "interlude_sea",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "候補には、私の名前を出します。"
        },
        {
          "id": "interlude_sea_012",
          "sceneId": "interlude_sea",
          "type": "dialogue",
          "speaker": "amane",
          "text": "最終画面に出すかは。"
        },
        {
          "id": "interlude_sea_013",
          "sceneId": "interlude_sea",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "見る方に選んでいただきますわ。"
        },
        {
          "id": "interlude_sea_014",
          "sceneId": "interlude_sea",
          "type": "dialogue",
          "speaker": "amane",
          "text": "公開版そのものは変えない。"
        },
        {
          "id": "interlude_sea_015",
          "sceneId": "interlude_sea",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "ええ。"
        },
        {
          "id": "interlude_sea_016",
          "sceneId": "interlude_sea",
          "type": "narration",
          "speaker": "narrator",
          "text": "タイマーが鳴る。"
        },
        {
          "id": "interlude_sea_017",
          "sceneId": "interlude_sea",
          "type": "dialogue",
          "speaker": "amane",
          "text": "戻るよ。"
        },
        {
          "id": "interlude_sea_018",
          "sceneId": "interlude_sea",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "はい。"
        },
        {
          "id": "interlude_sea_019",
          "sceneId": "interlude_sea",
          "type": "narration",
          "speaker": "narrator",
          "text": "立ち上がったあと、ミズハは青りんごを紙袋から出した。\n海へ向けて一枚だけ写真を撮り、袋へ戻した。"
        }
      ],
      "nextSceneId": "mode08_map_layers"
    },
    {
      "id": "mode08_map_layers",
      "title": "MODE 08｜地図モード／三つの生態系",
      "chapter": "MODE 08",
      "modeIndex": 7,
      "interaction": {
        "kind": "map08",
        "requiredLayers": [
          "nature",
          "life",
          "memory"
        ]
      },
      "steps": [
        {
          "id": "mode08_map_layers_001",
          "sceneId": "mode08_map_layers",
          "type": "ui",
          "text": "表示｜抽象モードから地図モードへ"
        },
        {
          "id": "mode08_map_layers_002",
          "sceneId": "mode08_map_layers",
          "type": "narration",
          "speaker": "narrator",
          "text": "波と時間だけになっていた画面へ、海岸線と町の名前が戻る。\n同じ場所に、三つの層が開く。"
        },
        {
          "id": "mode08_map_layers_003",
          "sceneId": "mode08_map_layers",
          "type": "narration",
          "speaker": "narrator",
          "text": "自然環境には森林と水域、人の暮らしには人口と夜間光、土地の記憶には地名と写真が入っている。"
        },
        {
          "id": "mode08_map_layers_004",
          "sceneId": "mode08_map_layers",
          "type": "interaction",
          "interaction": {
            "kind": "map08",
            "requiredLayers": [
              "nature",
              "life",
              "memory"
            ]
          },
          "text": "操作｜三つの層を切り替える"
        },
        {
          "id": "mode08_map_layers_005",
          "sceneId": "mode08_map_layers",
          "type": "narration",
          "speaker": "narrator",
          "text": "プレイヤーは自然環境、人の暮らし、土地の記憶を順に開き、同じ地点の違う記録を見る。"
        },
        {
          "id": "mode08_map_layers_006",
          "sceneId": "mode08_map_layers",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "数値のない欄もありますわね。"
        },
        {
          "id": "mode08_map_layers_007",
          "sceneId": "mode08_map_layers",
          "type": "dialogue",
          "speaker": "amane",
          "text": "ないなら、ないまま出す。"
        },
        {
          "id": "mode08_map_layers_008",
          "sceneId": "mode08_map_layers",
          "type": "narration",
          "speaker": "narrator",
          "text": "更新履歴を開く。"
        },
        {
          "id": "mode08_map_layers_009",
          "sceneId": "mode08_map_layers",
          "type": "narration",
          "speaker": "narrator",
          "text": "来場者欄の試作 / VISITOR FIELD PROTOTYPE\n編集者 / EDITORS：MIZUHA / AMANE / SAKUYA\n画面実装 / SCREEN：AMANE\n問いかけ / PROMPT：MIZUHA\n回答必須の解除 / OPTIONAL RESPONSE：SAKUYA\n状態 / STATUS：本編へ未接続"
        },
        {
          "id": "mode08_map_layers_010",
          "sceneId": "mode08_map_layers",
          "type": "narration",
          "speaker": "narrator",
          "text": "三人で試したまま、使い道を決めなかった欄だった。"
        },
        {
          "id": "mode08_map_layers_011",
          "sceneId": "mode08_map_layers",
          "type": "dialogue",
          "speaker": "amane",
          "text": "三人とも触ってる。"
        },
        {
          "id": "mode08_map_layers_012",
          "sceneId": "mode08_map_layers",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "本編へつなぐのは、私たちですわね。"
        },
        {
          "id": "mode08_map_layers_013",
          "sceneId": "mode08_map_layers",
          "type": "narration",
          "speaker": "narrator",
          "text": "二人は自分たちの変更として、エピローグへ接続した。"
        }
      ],
      "nextSceneId": "mode10_space"
    },
    {
      "id": "mode10_space",
      "title": "MODE 10｜宇宙モード／最後の受信文",
      "chapter": "MODE 10",
      "modeIndex": 9,
      "interaction": {
        "kind": "space10",
        "requiredGestures": 1
      },
      "steps": [
        {
          "id": "mode10_space_001",
          "sceneId": "mode10_space",
          "type": "ui",
          "text": "表示｜地図モードから宇宙モードへ"
        },
        {
          "id": "mode10_space_002",
          "sceneId": "mode10_space",
          "type": "narration",
          "speaker": "narrator",
          "text": "地図の海岸線が丸みを持ち、画面の端へ回り込む。\n視点が引くと、森林と雨、地震波、三つの層が、地球上の別々の場所に残る。"
        },
        {
          "id": "mode10_space_003",
          "sceneId": "mode10_space",
          "type": "interaction",
          "interaction": {
            "kind": "space10",
            "requiredGestures": 1
          },
          "text": "操作｜地球を回し、触れた記録を開く"
        },
        {
          "id": "mode10_space_004",
          "sceneId": "mode10_space",
          "type": "narration",
          "speaker": "narrator",
          "text": "プレイヤーが地球を回す。\nGX、MODE 03、07、08で触れた記録が、一つずつ点灯する。\n拡張版にあるほかの観測画面は、出典名だけが端に表示される。プレイヤーが触れた記録には数えない。"
        },
        {
          "id": "mode10_space_005",
          "sceneId": "mode10_space",
          "type": "narration",
          "speaker": "narrator",
          "text": "最初のバジルでLOCAL FIRSTを選んでいた場合、ミズハの部屋で湯が沸き、そのあとアマネの扇風機が鳴る。\nSTATION FIRSTを選んでいた場合、扇風機のあとに湯が沸く。"
        },
        {
          "id": "mode10_space_006",
          "sceneId": "mode10_space",
          "type": "narration",
          "speaker": "narrator",
          "text": "最後に、サクヤの窓の外で踏切が鳴る。"
        },
        {
          "id": "mode10_space_007",
          "sceneId": "mode10_space",
          "type": "narration",
          "speaker": "narrator",
          "text": "三人が年越しのハドルで黙っていた十一秒が再生される。\n声はない。湯の沸く音、扇風機、踏切が同じ録音に入っている。"
        },
        {
          "id": "mode10_space_008",
          "sceneId": "mode10_space",
          "type": "dialogue",
          "speaker": "mizuha",
          "text": "音量を下げます？"
        },
        {
          "id": "mode10_space_009",
          "sceneId": "mode10_space",
          "type": "dialogue",
          "speaker": "amane",
          "text": "このままで。"
        },
        {
          "id": "mode10_space_010",
          "sceneId": "mode10_space",
          "type": "narration",
          "speaker": "narrator",
          "text": "サクヤから最後に届いたメッセージが表示される。"
        },
        {
          "id": "mode10_space_011",
          "sceneId": "mode10_space",
          "type": "record",
          "recordType": "SOURCE",
          "text": "観測記録 / SOURCE"
        },
        {
          "id": "mode10_space_012",
          "sceneId": "mode10_space",
          "type": "chat",
          "time": "02:14",
          "speaker": "sakuya",
          "speakerLabel": "SAKUYA",
          "text": "もし地球の声が聞こえたと思ったら、すぐに意味を決めるんじゃなくて――"
        },
        {
          "id": "mode10_space_013",
          "sceneId": "mode10_space",
          "type": "narration",
          "speaker": "narrator",
          "text": "このあとに届いたメッセージ：0"
        },
        {
          "id": "mode10_space_014",
          "sceneId": "mode10_space",
          "type": "narration",
          "speaker": "narrator",
          "text": "その下に、MODE 07で見た青い文のカードが閉じた状態で置かれる。"
        },
        {
          "id": "mode10_space_015",
          "sceneId": "mode10_space",
          "type": "narration",
          "speaker": "narrator",
          "text": "制作記録 / PRODUCTION RECORD\n生成実行・選定責任 / RESPONSIBLE：MIZUHA\nサクヤ本人の確認：なし"
        },
        {
          "id": "mode10_space_016",
          "sceneId": "mode10_space",
          "type": "dialogue",
          "speaker": "amane",
          "text": "公開版のファイルは変えない。\nここで決めるのは、最後の画面に制作過程まで出すかどうか。"
        }
      ],
      "nextSceneId": "choice_editorial"
    },
    {
      "id": "choice_editorial",
      "title": "編集方針の選択｜最終画面に何を残すか",
      "chapter": "EDITORIAL CHOICE",
      "modeIndex": 9,
      "choice": {
        "id": "editorial_choice",
        "prompt": "最終画面に何を残すか",
        "trackedByEves": true,
        "options": [
          {
            "value": "SOURCE_RECORD",
            "next": "epilogue_visitor_field"
          },
          {
            "value": "DISCLOSE_DERIVATION",
            "next": "epilogue_visitor_field"
          }
        ]
      },
      "steps": [
        {
          "id": "choice_editorial_001",
          "sceneId": "choice_editorial",
          "type": "choice",
          "choiceId": "editorial_choice",
          "prompt": "最終画面に何を残すか",
          "trackedByEves": true,
          "options": [
            {
              "value": "SOURCE_RECORD",
              "next": "epilogue_visitor_field",
              "label": "本人記録で構成する / SOURCE RECORD"
            },
            {
              "value": "DISCLOSE_DERIVATION",
              "next": "epilogue_visitor_field",
              "label": "生成した制作過程も開示する / DISCLOSE DERIVATION"
            }
          ]
        },
        {
          "id": "choice_editorial_002",
          "sceneId": "choice_editorial",
          "condition": {
            "key": "editorialChoice",
            "value": "SOURCE_RECORD"
          },
          "type": "ui",
          "text": "本人記録で構成する / SOURCE RECORD"
        },
        {
          "id": "choice_editorial_003",
          "sceneId": "choice_editorial",
          "condition": {
            "key": "editorialChoice",
            "value": "SOURCE_RECORD"
          },
          "type": "narration",
          "speaker": "narrator",
          "text": "最終画面には、サクヤが送った白い文だけが残る。\n生成した事実は制作記録へ置き、本人の記録とは並べない。"
        },
        {
          "id": "choice_editorial_004",
          "sceneId": "choice_editorial",
          "condition": {
            "key": "editorialChoice",
            "value": "DISCLOSE_DERIVATION"
          },
          "type": "ui",
          "text": "生成した制作過程も開示する / DISCLOSE DERIVATION"
        },
        {
          "id": "choice_editorial_005",
          "sceneId": "choice_editorial",
          "condition": {
            "key": "editorialChoice",
            "value": "DISCLOSE_DERIVATION"
          },
          "type": "narration",
          "speaker": "narrator",
          "text": "白い原文の横に、ミズハが生成した青い文が別のカードで開く。"
        },
        {
          "id": "choice_editorial_006",
          "sceneId": "choice_editorial",
          "condition": {
            "key": "editorialChoice",
            "value": "DISCLOSE_DERIVATION"
          },
          "type": "record",
          "recordType": "DERIVED",
          "text": "計算・解釈 / DERIVED\n生成部分：「聞こえたつもりになってない？」って、三人で確かめたい。\n生成実行・選定責任 / RESPONSIBLE：MIZUHA\nサクヤ本人の確認：なし"
        },
        {
          "id": "choice_editorial_007",
          "sceneId": "choice_editorial",
          "type": "narration",
          "speaker": "narrator",
          "text": "セッション内の表示だけが変わる。\n学園祭公開版1.0と、次の来場者が最初に見る画面は変更されない。"
        },
        {
          "id": "choice_editorial_008",
          "sceneId": "choice_editorial",
          "type": "record",
          "recordType": "VISITOR_TRACE",
          "text": "操作記録 / VISITOR TRACE\n編集方針 / EDITORIAL CHOICE：SOURCE RECORD または DISCLOSE DERIVATION"
        },
        {
          "id": "choice_editorial_009",
          "sceneId": "choice_editorial",
          "type": "narration",
          "speaker": "narrator",
          "text": "選択を終えると、三人が試作した来場者欄が開く。"
        }
      ],
      "nextSceneId": "epilogue_visitor_field"
    },
    {
      "id": "epilogue_visitor_field",
      "title": "EPILOGUE｜書き込み欄 / WRITE ACCESS",
      "chapter": "EPILOGUE",
      "modeIndex": 9,
      "steps": [
        {
          "id": "epilogue_visitor_field_001",
          "sceneId": "epilogue_visitor_field",
          "type": "narration",
          "speaker": "narrator",
          "text": "隠された文章は出ない。\n三人が以前作った来場者欄と、その後の変更者だけが表示される。"
        },
        {
          "id": "epilogue_visitor_field_002",
          "sceneId": "epilogue_visitor_field",
          "type": "narration",
          "speaker": "narrator",
          "text": "来場者欄の試作 / VISITOR FIELD PROTOTYPE\n編集者 / EDITORS：MIZUHA / AMANE / SAKUYA\n回答必須の解除 / OPTIONAL RESPONSE：SAKUYA\n本編への接続 / CONNECTED BY：MIZUHA / AMANE"
        },
        {
          "id": "epilogue_visitor_field_003",
          "sceneId": "epilogue_visitor_field",
          "type": "narration",
          "speaker": "narrator",
          "text": "古いテストでは、文章を書かなかった人が先へ進めなかった。\nサクヤは回答必須を解除し、「書かない人もいる」とだけ返していた。"
        },
        {
          "id": "epilogue_visitor_field_004",
          "sceneId": "epilogue_visitor_field",
          "type": "narration",
          "speaker": "narrator",
          "text": "書き込み欄 / WRITE ACCESSは、サクヤの文章へ続きを足す欄ではない。\nプレイヤーが自分の名義で一行を表示するための欄だった。"
        },
        {
          "id": "epilogue_visitor_field_005",
          "sceneId": "epilogue_visitor_field",
          "type": "narration",
          "speaker": "narrator",
          "text": "サクヤの吹き出しから一行離れた場所に、入力欄が表示される。"
        },
        {
          "id": "epilogue_visitor_field_006",
          "sceneId": "epilogue_visitor_field",
          "type": "record",
          "recordType": "VISITOR_POST",
          "text": "投稿者 / AUTHOR：VISITOR\n分類 / TYPE：来場者の投稿 / VISITOR POST\nサクヤの文への接続 / LINK：NO"
        },
        {
          "id": "epilogue_visitor_field_007",
          "sceneId": "epilogue_visitor_field",
          "type": "narration",
          "speaker": "narrator",
          "text": "入力欄の上には、一行だけ表示される。"
        },
        {
          "id": "epilogue_visitor_field_008",
          "sceneId": "epilogue_visitor_field",
          "type": "narration",
          "speaker": "narrator",
          "text": "「この展示を見た現在の記録を、残したい場合だけ一行書いてください。」"
        },
        {
          "id": "epilogue_visitor_field_009",
          "sceneId": "epilogue_visitor_field",
          "type": "ui",
          "text": "START前に示した保存方針が、入力欄の下へ再表示される。"
        },
        {
          "id": "epilogue_visitor_field_010",
          "sceneId": "epilogue_visitor_field",
          "type": "narration",
          "speaker": "narrator",
          "text": "120文字まで／このセッションだけ／サーバー送信なし／次の人に本文は見えない"
        },
        {
          "id": "epilogue_visitor_field_011",
          "sceneId": "epilogue_visitor_field",
          "type": "ui",
          "text": "保存方針を詳しく見る"
        },
        {
          "id": "epilogue_visitor_field_012",
          "sceneId": "epilogue_visitor_field",
          "type": "narration",
          "speaker": "narrator",
          "text": "- HTMLとスクリプトは実行せず、入力された文字として表示する。\n- 本名、住所、連絡先を書かない。\n- STARTへ戻るか「この文章を消す」を押すと本文を削除する。"
        },
        {
          "id": "epilogue_visitor_field_013",
          "sceneId": "epilogue_visitor_field",
          "type": "visitorInput",
          "inputId": "visitor_post",
          "maxLength": 120
        }
      ],
      "nextSceneId": "choice_visitor_action"
    },
    {
      "id": "choice_visitor_action",
      "title": "最後の選択｜自分の記録を置くか",
      "chapter": "FINAL CHOICE",
      "modeIndex": 9,
      "choice": {
        "id": "visitor_action",
        "prompt": "自分の記録を置くか",
        "trackedByEves": true,
        "options": [
          {
            "value": "WRITE",
            "next": "final_record"
          },
          {
            "value": "LEAVE_EMPTY",
            "next": "final_record"
          }
        ]
      },
      "steps": [
        {
          "id": "choice_visitor_action_001",
          "sceneId": "choice_visitor_action",
          "type": "choice",
          "choiceId": "visitor_action",
          "prompt": "自分の記録を置くか",
          "trackedByEves": true,
          "options": [
            {
              "value": "WRITE",
              "next": "final_record",
              "label": "一行書く / WRITE"
            },
            {
              "value": "LEAVE_EMPTY",
              "next": "final_record",
              "label": "何も書かずに終える / LEAVE EMPTY"
            }
          ]
        },
        {
          "id": "choice_visitor_action_002",
          "sceneId": "choice_visitor_action",
          "condition": {
            "key": "visitorAction",
            "value": "WRITE"
          },
          "type": "ui",
          "text": "一行書く / WRITE"
        },
        {
          "id": "choice_visitor_action_003",
          "sceneId": "choice_visitor_action",
          "condition": {
            "key": "visitorAction",
            "value": "WRITE"
          },
          "type": "narration",
          "speaker": "narrator",
          "text": "入力欄が有効になる。\nプレイヤーが120文字以内で文章を書く。"
        },
        {
          "id": "choice_visitor_action_004",
          "sceneId": "choice_visitor_action",
          "condition": {
            "key": "visitorAction",
            "value": "WRITE"
          },
          "type": "narration",
          "speaker": "narrator",
          "text": "送信すると、文章はサクヤの吹き出しへつながらず、VISITORの新しい投稿として表示される。"
        },
        {
          "id": "choice_visitor_action_005",
          "sceneId": "choice_visitor_action",
          "condition": {
            "key": "visitorAction",
            "value": "WRITE"
          },
          "type": "record",
          "recordType": "VISITOR_POST",
          "text": "来場者の投稿 / VISITOR POST\n作者 / AUTHOR：VISITOR\n表示：STARTへ戻るまで"
        },
        {
          "id": "choice_visitor_action_006",
          "sceneId": "choice_visitor_action",
          "condition": {
            "key": "visitorAction",
            "value": "LEAVE_EMPTY"
          },
          "type": "ui",
          "text": "何も書かずに終える / LEAVE EMPTY"
        },
        {
          "id": "choice_visitor_action_007",
          "sceneId": "choice_visitor_action",
          "condition": {
            "key": "visitorAction",
            "value": "LEAVE_EMPTY"
          },
          "type": "narration",
          "speaker": "narrator",
          "text": "入力欄は空のまま閉じる。\n文章を送らなかった操作だけが残る。"
        },
        {
          "id": "choice_visitor_action_008",
          "sceneId": "choice_visitor_action",
          "type": "narration",
          "speaker": "narrator",
          "text": "どちらにも「正解」の印は付かない。"
        }
      ],
      "nextSceneId": "final_record"
    },
    {
      "id": "final_record",
      "title": "最終表示｜誰が何を残したか",
      "chapter": "FINAL RECORD",
      "modeIndex": 9,
      "steps": [
        {
          "id": "final_record_001",
          "sceneId": "final_record",
          "type": "record",
          "recordType": "SOURCE",
          "text": "サクヤの受信文 / SAKUYA SOURCE：1\n編集方針 / EDITORIAL CHOICE：SOURCE RECORD または DISCLOSE DERIVATION\n来場者の操作 / VISITOR ACTION：WRITE または LEAVE EMPTY\n公開版の変更 / PUBLIC BUILD CHANGED：NO"
        },
        {
          "id": "final_record_002",
          "sceneId": "final_record",
          "type": "narration",
          "speaker": "narrator",
          "text": "画面は現在の展示ホールへ戻る。"
        },
        {
          "id": "final_record_003",
          "sceneId": "final_record",
          "type": "narration",
          "speaker": "narrator",
          "text": "プレイヤーが文章を書いていた場合、その一行はまだ自分の画面に見えている。\n「この文章を消す」を押すか、STARTへ戻ると本文は消える。"
        },
        {
          "id": "final_record_004",
          "sceneId": "final_record",
          "type": "narration",
          "speaker": "narrator",
          "text": "アマネが端末をSTARTへ戻した。"
        },
        {
          "id": "final_record_005",
          "sceneId": "final_record",
          "type": "narration",
          "speaker": "narrator",
          "text": "WRITEを選んでいた場合、黄色い点が一つ増える。\nLEAVE EMPTYを選んでいた場合、黄色い細い円が一つ増える。\nどちらにも本文は入っていない。"
        },
        {
          "id": "final_record_006",
          "sceneId": "final_record",
          "type": "narration",
          "speaker": "narrator",
          "text": "ミズハは青りんごの向きを変え、傷のない側を通路へ向けた。"
        },
        {
          "id": "final_record_007",
          "sceneId": "final_record",
          "type": "narration",
          "speaker": "narrator",
          "text": "館内放送が、次のステージ企画を告げる。\n隣のブースで拍手が起きる。"
        },
        {
          "id": "final_record_008",
          "sceneId": "final_record",
          "type": "narration",
          "speaker": "narrator",
          "text": "最初のプレイヤーが席を立つ。\n通路を歩いていた次の人が、青りんごの前で足を止めた。"
        },
        {
          "id": "final_record_009",
          "sceneId": "final_record",
          "type": "ui",
          "text": "START\n────────────────────────────────────────\n— END OF PLAYER STORY —\n────────────────────────────────────────"
        },
        {
          "id": "final_record_010",
          "sceneId": "final_record",
          "type": "ui",
          "text": "CLEAR後｜制作記録を見る"
        },
        {
          "id": "final_record_011",
          "sceneId": "final_record",
          "type": "result",
          "resultId": "session_result"
        }
      ],
      "nextSceneId": "return_to_start"
    },
    {
      "id": "return_to_start",
      "title": "END OF PLAYER STORY",
      "chapter": "END",
      "modeIndex": 9,
      "steps": [
        {
          "id": "return_to_start_001",
          "sceneId": "return_to_start",
          "type": "end",
          "text": "START"
        }
      ],
      "nextSceneId": null
    }
  ]
});
