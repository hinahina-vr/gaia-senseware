...
3. `MAP 26—30` で、データ時刻、現在時刻からの経過、ライブ／キャッシュ／保存値の状態を確認します。
...
| モード | 内容 |
|---|---|
| `MAP` | 公開データを重ねる地図展示30本。保存データ、国内統計、ライブ／モデル値を同じUIで比較 |
...
| 16—25 | 日本の公的統計 | 人の潮目、旅の灯、住まいの芽吹き、空の体温、日最高気温の年平均、日最低気温の年平均、湿りの膜、光の貯金、雨の器、雨の足跡 |
| 26 | 全球・直近24時間 | 燃える惑星 — NASA FIRMSの火災・熱異常 |
| 27—30 | 全球・ライブ公開データ | 大気をなぞる、大気の散乱、地殻の波紋、雲を透る光 |
...
| 10—15 | Cloudflare Pages APIの `/api/live/v1/snapshot`、`/stream`、`/wind-field`。現在の展示値はOpen-MeteoとCAMS | 提供元、地点、JSTのデータ時刻、更新方法、ライブ／保存値の状態を表示 |
| 16—25 | e-Statと気象庁から生成した `data/estat-prefecture-series.json` | 月次・年次・1955〜2025年の長期系列を、47都道府県で比較。閲覧時の外部API通信なし |
| 26 | Pages APIの `/api/live/v1/firms` からNASA LANCE FIRMSを取得 | 取得時刻と観測時刻を分け、JSTの最新時刻と経過時間を表示。失敗時は期限切れキャッシュまたは版管理スナップショット |
| 27—30 | ブラウザからOpen-MeteoとUSGSの公開APIを直接取得 | JSTのデータ時刻と経過時間、ライブ／5分キャッシュ／演出用サンプルの状態を表示 |
| ORBITAL | `data/space-signals.json` | 閲覧時は外部APIへ接続せず、収録期間・取得日時・単位・加工を表示 |

公開版は閲覧中に外部APIへ接続します。「外部JavaScriptランタイムライブラリを使わない」ことと「外部データAPIを使わない」ことは別です。

`LIVE CACHE` は「今この瞬間の実測」や常時更新を保証する表示ではありません。MAP 27—30では取得後5分以内のタブ内キャッシュ、MAP 26ではサイト側APIの取得値（サーバーでは15分を基準にキャッシュ）を表します。取得時刻と観測・モデルのデータ時刻は異なるため、画面のデータ時刻と経過時間も確認してください。

取得失敗時の扱いは展示ごとに異なります。MAP 01は保存スナップショット、MAP 10—15は最後の取得値・対応地点の保存値または欠測、MAP 26は期限切れAPIキャッシュまたは同梱スナップショットへ切り替えます。MAP 26では両方が `SAVED SNAPSHOT` 表示です。MAP 27—30の `SAVED VALUES` は演出用サンプルであり、過去の実測データではありません。詳しくは[取得状態の読み方](docs/ARCHITECTURE.md#取得状態の読み方)を参照してください。
MAP 26—30では基礎展示のPOI判定を停止し、地図を押しても別展示の詳細カードを開きません。

## データが表現になるまで

| 区分 | 意味 | 例 |
|---|---|---|
| `○ SOURCE / 公開記録` | 提供元が公開した観測・統計・モデル値 | CO₂濃度、気温、風速、雲量、日射、震源 |
| `△ DERIVED / 計算・補間` | 元データから計算した値 | 欠測補完、線形補間、正規化、集計、表示用の尺度変換 |
| `◇ SCENARIO / 仮定・操作` | 仮定または観客操作で作る状態 | 2026〜2050年の線形トレンド試算、一定海流を仮定した移動距離 |
| `VISUAL / 表現` | 数値を画面へ割り当てた結果 | 風速→筆触、FRP→粒径、地震規模→波紋、日射→光点 |
...
GitHub Actionsの `Contest checks` は実Google Chromeで、初期転送量、PC／モバイルレイアウト、遅延読込、直接URL、履歴・再読込、WebGLフォールバック、30展示の主要導線、JavaScriptエラー、未処理Promise、404を検査します。CIからpushやデプロイは行いません。
...
src/exploration/                  MAP 10—30、ライブ取得、LOD
