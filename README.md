...
実際の描画コードを確認できます。第10章には、それまでに表示した九つのデータと、観客が
画面に触れた跡が集まります。
...
| 04 | **共進化プロトコル** | GloBI / GBIF | 花と虫の関係を線に、ミツバチの観察場所を点にする |
| 05 | **すべては次の資源** | UN SDG 12.5.1 | 再資源化率を、公式値・計算した値・観客の試算に分ける |
| 06 | **人類世の傷跡** | EDGAR / NASA VIIRS | 夜の明かりを白、国ごとの温室効果ガス排出量を赤で重ねる |
| 07 | **地球からのメッセージ** | 気象庁 / USGS | 世界の大地震と、日本各地で実際に記録された震度を表示する |
| 08 | **三つの生態系** | NASA MODIS / World Bank / UNESCO | 森林、都市人口、各地域から選んだ世界遺産を三層で表示する |
| 09 | **人工物の共生化** | NASA POWER / World Bank | 日差しや風と、現在の再生可能電力の割合を見比べる |
| 10 | **未完の地球センスウェア** | 01〜09を再利用 | 九つのデータと、観客が画面に触れた跡を重ねる |
...
04は62生物観察記録、05は17地域の公式値＋14地域の補完値、06は31か国、07は世界の142件から年代別に選んだ24件、
...
`STORY`を開くと、10の感覚器を背景にしたオリジナルのデータ・フィクション
...
`SOURCE / DERIVED / SCENARIO`の区別へ接続します。物語を進めると背後の演出も01から10へ実際に切り替わります。

登場人物と出来事はすべてフィクションです。科学データの出典区分と物語上の出来事を混同しないよう、
画面には常に`DATA FICTION`と、各場面に対応するデータ区分を表示します。
...
- `STORY`でデータ・フィクションを開き、クリック／タップ／Spaceで読み進める
...
- `TOP` またはキーボードの `I` で、10テーマを選ぶ最初の画面へ戻る
