...
`.trim(),
    },
    {
      ...
      titleJa: "地球からのメッセージ",
      description: "USGSの世界M7.5以上を2000〜2026年まで自動で送り、各年の震源を発生日時順に一つずつ表示します。点に続いて輪が広がり、Magnitudeから見積もった可感半径の目安で止まります。輪は実際の震度分布・被害範囲・津波範囲ではありません。気象庁の日本実測震度は別層です。",
      accent: "#ffb45f",
      rgb: "255, 180, 95",
      source: `
...
`.trim(),
  ...
  const modeConcepts = {
    ...
    "rhythm-of-disaster": {
      lead:
        "世界の大地震を全部重ねず、2000〜2026年を約4.6秒ずつ自動で送ります。年によって震源の数と場所がどう変わるかを見ます。",
      seeing:
        "橙の点は選択年度にUSGSが記録したM7.5以上の震源だけです。発生日時の早い地震から順に点と輪が立ち上がり、M7.5は約500km、M9.1は約2,000kmの推定可感半径で止まります。別年度の点は表示しません。",
      touch:
        "年度は自動で進み、切替時には旧年を消して新年の最初の地震から順番に再生します。スライダーで任意の年へ移動でき、震源を押すと発生日・深さ・Magnitudeを読めます。日本の震度6弱以上の実測記録は、左下の切替から別層で見られます。",
      context:
  ...
  const modeDataNarratives = Object.freeze({
    ...
    "rhythm-of-disaster": "地図の見方：初期表示は世界です。2000〜2026年を約4.6秒ずつ自動再生し、その年のUSGS M7.5以上を発生日時の早い順に一つずつ表示します。年度切替時は新年の最初の震源から点と推定可感半径の輪が順番に立ち上がります。実際の震度分布・被害・津波範囲ではなく、日本の実測震度は別層です。",
