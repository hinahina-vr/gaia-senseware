(() => {
  ...
  const INTRO_MODE_CHOICES = [
    ...
    },
    {
      ...
      cue: "九つの測定",
      code: "DATA ATLAS",
      copy: "測るもの・代表値・単位を9枚で同時に比べ、ひとつの総合点にはしない。",
    },
  ];
...
`.trim(),
    },
    {
      ...
      titleJa: "地球からのメッセージ",
      description: "USGSの世界M7.5以上を2000〜2026年の年度ごとに切り替えます。その年の全震源から同心円が一斉に広がり、Magnitudeが大きいほど最大半径が大きくなります。円は比較演出で、実際の揺れの範囲ではありません。気象庁の日本実測震度は別層です。",
      accent: "#ffb45f",
      rgb: "255, 180, 95",
      source: `
...
`.trim(),
    ...
    {
      id: "senseware-2050",
      title: "Nine Measures, No Single Score",
      titleJa: "九つの測定は、足せない",
      description: "01〜09が何を測っているのか、実際の代表値と単位を3×3のカードで同時に表示します。ppm、m/s、mm/day、%、件数、相関係数は意味が違うため、ひとつの『地球スコア』には足しません。",
      accent: "#c8f7ff",
      rgb: "200, 247, 255",
      source: `
...
`.trim(),
  ...
  const modeConcepts = {
    ...
    "rhythm-of-disaster": {
      ...
      seeing:
        "橙の点は選択年度にUSGSが記録したM7.5以上の震源だけです。年度が変わると全震源から輪が同時に広がり、M9.1の最大波は地図幅のおよそ半分まで届きます。別年度の点は表示しません。",
      touch:
        "スライダーで年度を切り替えると、その年の輪が一斉に始まります。震源を押すと発生日・深さ・Magnitudeを読めます。日本の震度6弱以上の実測記録は、左下の切替から別層で見られます。",
      context:
    ...
    "senseware-2050": {
      lead:
        "地球の状態を、ひとつの数字だけで表せるでしょうか。最後の展示では、ここまでの九つが何を測っていたのかを一画面で並べます。",
      seeing:
        "3×3の各カードに、測るもの・データから取り出した代表値・単位または標本数を表示します。01のppm、02のm/s、03のmm/day、割合、件数、相関係数rが、同じ種類の数字ではないことが一目で分かります。",
      touch:
        "9枚は最初から同時に表示されます。スライダーを動かすと黄色い枠が01〜09へ移り、選んだカードの実値と対象範囲を左下・右下で大きく読めます。",
      context:
        "授業「地球大の感覚神経系を獲得した人類」が、この作品の出発点です。観測を増やすほど地球はよく見えますが、意味の違う数値を足した総合点は、重要な違いや不確かさを隠します。",
      question: "九つのうち、何と何なら一緒に読めるでしょう。反対に、足してはいけない値はどれでしょう。",
    },
  };
  ...
  const modeDataNarratives = Object.freeze({
    ...
    "rhythm-of-disaster": "地図の見方：初期表示は世界です。2000〜2026年を年度ごとに切り替え、その年のUSGS M7.5以上だけを表示します。年度切替時に全震源の輪が一斉に広がり、M9.1では地図幅の約半分まで達します。輪はMagnitude比較用で、実際の揺れや震度ではありません。日本の実測震度は別層です。",
    ...
    "senseware-2050": "画面の見方：3×3の9枚すべてが同時に見えます。各カードは、測るもの・データから取り出した代表値・単位または標本数の組です。黄色い枠だけがスライダーで移動します。ppm、m/s、mm/day、%、件数、相関係数rは意味が違うため、合計や総合順位にはしません。",
  });
